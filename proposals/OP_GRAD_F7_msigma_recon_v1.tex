...
unique dimensionally consistent gradient-native gap \\
...
Mixed import ($u^2/\phi_0^2$) & $\propto u^2$ & $0$ & \emph{Excluded by the
data}: switches the $\varepsilon$-sector off, in conflict with the retained
five-probe band \\
\end{longtable}\normalsize
...
potential-imported gap --- with the third branch already excluded by the
$\varepsilon$-band itself. Each answer has a definite, bounded consequence;
...
excursion ($|\Delta\ln m_\sigma^2|=|\Delta\ln u|\lesssim0.05$): the D2
pinning logic extends to $\sigma$, which cannot roll, and the closure
acquires no additional light degree of freedom.
...
required slope both fixed by the record and the third branch excluded by the
$\varepsilon$-band.
...
unique dimensionally consistent gap; the coefficient is fixed as
