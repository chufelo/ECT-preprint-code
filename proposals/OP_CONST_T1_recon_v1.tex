...
together: since $S_0$ is recorded as a protected structural invariant,
\[
\zeta_{K_\theta}\;=\;\zeta_{m_\varphi^2}
\qquad\text{(equal background-tracking exponents)},
\]
up to the kinematic factor $c_\perp$. \textbf{Mini no-go (asymmetric-tracking
lemma, candidate formal statement):} if $K_\theta$ tracked the background while
$m_\varphi^2$ did not (or vice versa), $S_0^{\rm EFT}$ would drift, contradicting
its recorded universality. The recorded structure therefore already \emph{forbids}
asymmetric tracking of the pair $(K_\theta,m_\varphi^2)$.
...
$u_0$ --- the same OP-grad object as in \S2(c). Hence \emph{both} electroweak
kinetic channels reduce to one and the same anchoring question. Corollary upgrade:
since both channels now reduce to the anchoring of a single scale, common tracking
is the generic case, and the Step-3 ratio-protection of
$\sin^2\theta_W=Z_W/(Z_B+Z_W)$ holds in \emph{either} outcome (P-anchored: both
frozen; B-tracking: common factor cancels), up to the recorded structural
difference between the two stiffness coefficients.
...
the $S_0$ lever: the recorded identity plus $S_0$-universality enforce equal
tracking of $(K_\theta,m_\varphi^2)$ and reduce the question to the anchoring of
the evaluation scale, with the parameter-level reading
($m_\varphi=\sqrt{2\lambda}\,\phi_0$) explicitly recorded in the simplest
realisation.
\item The remaining gaps are exactly two, both already named in the preprint:
(C1)~the multiplicative inheritance $Z_B^{(0)}\!\leftarrow\!K_\theta^{\rm eff}$
must be computed explicitly (the hard core of open target (vii)); (C2)~the
$u_0\!\leftrightarrow$ bare-data closure (OP-grad). \emph{Outcome A is now a
conditional protection-theorem sketch with two named conditions --- not
established, but well-posed.} Outcome C remains the quantified fallback of Step 3
(channel-dependent $\varepsilon$ bounds).
...
(T1c)~Formalise the asymmetric-tracking no-go of \S2(b) as a lemma --- a cheap,
clean candidate theorem. (T1d)~The $\mu$-side selection principle of Step 3B
