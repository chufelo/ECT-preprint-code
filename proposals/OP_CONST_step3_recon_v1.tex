...
\date{12 June 2026 --- rev.~1.1 (review softenings applied; symmetric anchoring framing)}
...
\textbf{(b) Consequence for the dichotomy.} The recorded stiffness-induced
composition makes $u$-tracking a serious live danger \emph{if} the stiffness scale
is identified with the drifting background amplitude $u(z)$. However, the current
record does not yet decide whether the relevant stiffness is P-anchored at
$\phi_0=\sqrt{\mu^2/\lambda}$ (parameter-level, frozen) or B-tracking with $u(X)$;
the $\sim\phi_0^2$ scalings as written can support either reading. Deciding this is
precisely the T1 target, and the question is symmetric: neither protection nor
tracking is established by the record.
...
$(Z_B^{(0)},Z_W^{(0)})$ --- the power-law channel, $p\simeq2$ in the tracking
scenario;
...
$p=2$ stiffness tracking (stress scenario) & $2$ & $0.12$ & $\sim10^{5}$ tension \\
...
\beta_{\rm eff}$ --- a ratio of fixed-point data. This template is a \emph{naturally P-anchored candidate, provided the fixed-point
data and thresholds are background-independent} (and subject to the recorded
$\gamma_m\gtrsim1$ protection requirement).
...
\item \emph{Topological / Hopf-like}: $\mathcal I_{\rm EW}=S_{\rm top}$:
\emph{protected only if the action is genuinely topological and
coefficient-quantised, not merely geometric} --- a metric/stiffness prefactor
would reintroduce background dependence.
\end{enumerate}
...
$\{1;\ \approx0\ (\text{conditional, see (ii)});\ 2.5\times10^{3}\cdot\partial_{\ln
u}\delta;\ 0\ (\text{if coefficient-quantised})\}$. \emph{The filter favours the fixed-point
...
\textbf{Outcome A} (structural protection) requires actual theorems, not default
readings: for $\alpha$, a resolution of the stiffness-anchoring question via a
mechanism of class (P1)--(P3); for $\mu$, restriction of the
...
\textbf{Outcome C} (limitation) is quantified channel-dependently: $O(1)$ tracking
would force $\varepsilon_\alpha\lesssim5\times10^{-7}$ (quasar $\alpha$) and
$\varepsilon_\mu\lesssim4\times10^{-8}$ (H$_2$ $\mu$) under the quoted stress
anchors --- bounds of order $10^{-7}$--$10^{-6}$ and few$\times10^{-8}$
respectively, with the $\mu$ channel the stronger of the two. The retained
five-probe band $[0.0296,0.0376]$ would then be in tension by factors
$\sim5\times10^{4}$ ($\alpha$) to $\sim7\times10^{5}$ ($\mu$); the
$\varepsilon$-sector would not survive in its present form. Honest status line:
\emph{nothing in the current record establishes the protection; equally, nothing
establishes the tracking. Resolving this anchoring question is now the sharpest
open consistency requirement of the theory, and it was found internally rather
than by referees.}
