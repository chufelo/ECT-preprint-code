...
\subsection*{A2.4\quad Robustness check of the time-budget ansatz}

The linear form \eqref{eq:R_time} is the simplest defensible
choice, but the true stellar-mass-vs-time dependence is not known
at this precision. Two limiting cases bracket the physical
possibilities:
\begin{itemize}
...
(approximately $\pm 0.025$). The time-budget ansatz therefore does
not dominate the extracted interval; it is a secondary but
physically required correction.
...
Varying $z_{\rm form} \in [15, 25]$ shifts $R_{\rm time}$ by a
factor of order a few per cent; this shift is absorbed into the
robustness bracket of A2.4 and does not materially modify the
$1\sigma$ interval.
