...
categories whose reasons for exclusion are qualitatively different.
...
structure. The probe is therefore not used in the joint effective
band at the present stage and is deferred to a future full-likelihood
treatment.
...
is therefore not used in the joint effective band at the present stage
and is deferred to a future Boltzmann-level treatment.
\end{itemize}
...
excluded within the present uniform-$\varepsilon$ analysis on grounds
that are not reducible to a mere extraction-pipeline upgrade.
\end{itemize}
