...
controls simultaneously the late-time gravitational response function
$G_{\rm eff}(z)$ seen by every cosmological channel. The section
...
late-time potential-evolution signals) independently admit the same
narrow $\varepsilon$-interval, then that interval is not a fit of a
...
band with five channels of different kernels and systematics is the
strongest non-derivational evidence available at this methodological
level that a single underlying mechanism is at work.
...
reading; the detailed per-probe derivations, the extraction logic, the
uncertainty budgets, and the construction of each one-dimensional
$\varepsilon$-interval are collected in the dedicated methodology
appendices referenced from the subsections below. Channels not
