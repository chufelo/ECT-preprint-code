...
%   interface requirements; possible routes include
%   configuration-space measure/saddle selection and, more
%   speculatively, stochastic-quantisation-type relaxational
%   formalisms);"
...
Derive the ordered-branch EFT data $Z_u$, $\mathcal C_n$, $W(u)$
...
Establish dominance of the ordered sector within the recorded
...
implied).}
Any candidate ordering mechanism must: presuppose no emergent-time
or causal-cone structure prior to ordering; formulate selection as
Euclidean measure concentration rather than time evolution; derive
the ordered sector~(P4) and the EFT data rather than assume them;
hand $\rho_{\rm onset}$ to the thermal-completion ledger only
after an ordered Lorentzian branch exists, with no energy
statements at the pre-emergent Euclidean level; and respect the
recorded no-ghost expectations and the no-phantom
bound~\eqref{eq:w_phi}.
