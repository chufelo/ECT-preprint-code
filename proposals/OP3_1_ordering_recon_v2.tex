% ============================================================
% OP3_1_ordering_recon_v2.tex  (PROPOSAL-ONLY; v2 per GPT review)
...
\emph{Apparatus:} the recorded $S_0$-weighted Euclidean
extremal-action framework, with a saddle-dominance criterion when
$|\Delta S_E|/S_0\gg1$ (\S qs\_extremal\_action): Euclidean weight
...
(M1--M6; interface requirements, no mechanism; article-facing
status: ``OP3.1 remains Open; interface requirements recorded'' ---
explicitly NOT a Level~B result)}
...
(a)~existence of ordered stationary configurations, extrema, or
minimisers where the bare variational problem makes these notions
well-defined;
...
is equivalent to deriving $F,\omega,U$ \emph{within the already
recorded closure-function map and its normalisation freedoms}.
...
stationary configurations, extrema, or minimisers (where the bare
variational problem makes these notions well-defined) on the
constrained configuration space; (ii)~dominance/measure concentration of the
ordered sector; (iii)~extraction of the EFT data of M3(c) by
controlled expansion.
\emph{Conditional existence scaffold (proposal-only; NOT
article-facing before A0; for the article after A0 at most: ``a
future OP3.1a must specify whether the bare Euclidean functional
admits a well-posed variational problem suitable for existence and
dominance analysis'').}
...
Route~C remains proposal-only and should not be inserted in the
first OP3.1 interface update.
...
deliverable, with status ``Open; interface requirements recorded''
(NOT Level~B bookkeeping)?
