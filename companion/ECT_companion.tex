...
\textbf{Weak interaction (\emph{Level~B+}):} The residual rotation group $O(3)$ after $O(4) \to O(3)$ SSB is algebraically isomorphic to $SU(2)$ (as Lie algebras: $\mathfrak{so}(3) \cong \mathfrak{su}(2)$). The oriented condensate---through the sign of the four-volume form---selects one of the two $SU(2)$ factors of $Spin(4) \simeq SU(2)_+ \times SU(2)_-$, structurally selecting a single \emph{chiral} $SU(2)$ sector. This provides a structural explanation for why the weak interaction is maximally parity-violating: in the Standard Model this is a postulate; in ECT it follows from the orientation of the condensate. The phenomenological realisation occurs through a secondary condensate scale at $v_2 \simeq 246$\,GeV, producing $W^\pm$, $Z$ bosons and electroweak symmetry breaking. The full derivation of the weak sector as an independent internal gauge field remains open.

\textbf{Strong interaction (\emph{Level~B}):} $SU(3)$ colour \emph{cannot} arise from the minimal $O(4) \to O(3)$ pattern: the algebra $\mathfrak{su}(3)$ has dimension~8, while $\mathfrak{so}(4)$ has dimension~6, so $\mathfrak{su}(3)$ is simply too large to fit inside $\mathfrak{so}(4)$. This is not merely a technical obstruction---it is an algebraic impossibility. The electroweak-type $U(1) \times SU(2)$ architecture is the \emph{maximal} gauge structure derivable from the minimal condensate symmetry breaking. Any colour sector therefore requires additional internal degrees of freedom: a local complex rank-3 module with $SU(3)$-structure. A deeper geometric candidate ($G_2$ internal fibre) is also identified. Full QCD derivation remains open.
...
ECT thus provides a common pre-geometric origin for all known interaction sectors: gravity, electromagnetism, the weak and strong forces, and the director coupling all emerge from one condensate substrate, through distinct but co-necessary mechanisms. This is broader than conventional Grand Unified Theories ($SU(5)$, $SO(10)$), which embed only gauge interactions into a simple group while leaving gravity outside. ECT addresses gravity, parity violation, and the director coupling within the same framework. Its notion of unification is structural/pre-geometric rather than simple-group theoretic.

\subsection{Elementary particles}

The bosonic spectrum is organised hierarchically across four structural levels. At the minimal level (Level~0: real scalar condensate with $O(4) \to O(3)$ SSB), ECT contains the graviton (spin-2, \emph{Level~A/B}), the surviving Goldstone mode $\chi$ (ultra-light condensate scalar, \emph{Level~A/B}), and the radial mode $\sigma$ (heavy, \emph{Level~A/B})---no gauge bosons. At Level~1, the compact phase fibre produces the photon ($U(1)$ gauge boson, \emph{Level~A}). At Level~2, the electroweak condensate at $v_2 = 246$\,GeV generates $W^\pm$, $Z$, and the Higgs boson (\emph{Level~B+}). At Level~3, extended internal structure produces the eight gluons (\emph{Level~B}). Fermions arise as $O(3)$ spinorial excitations with a reconstructed Dirac equation (\emph{Level~A/B}), and topological sectors yield an instanton mass pattern $m_n = n \times 1.6$\,GeV/$c^2$ (\emph{Level~B}).

Relative to the Standard Model, ECT retains an additional condensate scalar sector: the ultra-light $\chi$-mode and the heavy radial mode $\sigma$. The detailed dark-sector role of $\chi$ remains model-dependent in the current formulation.
...
\midrule
Photon as $U(1)$ gauge boson & Postulated gauge principle & Compact phase $\theta$ with Maxwell-type leading kinetic term & A/B \\
$SU(2)_L$ electroweak sector & Postulated & Algebraic embedding $\mathfrak{so}(3)\cong\mathfrak{su}(2)$ & B/C \\
$SU(3)$ colour sector & Postulated & Candidate colour-like completion of the ordered branch & C \\
\midrule
...
\item \textbf{Anomaly cancellation (OP-GUT6).} Any chiral $SU(2)$ sector must be checked for both local triangle anomalies and the global Witten $\mathbb{Z}_2$ anomaly. These become sharp consistency tests once the fermion spectrum is derived.
