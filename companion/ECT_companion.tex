...
Born rule $P=|\psi|^2$ & Postulate & Conditional Gleason route under (C1)--(C3) + decoherence & B / C \\
...
\item \textbf{The measurement problem.} Decoherence and PES remove the need for a fundamental collapse postulate, but the full Born-rule closure remains conditional (\emph{B/Open}).

\item \textbf{The origin of quantum mechanics.} ECT provides a structural route to Hilbert space, superposition, and quantum dynamics from the condensate; the Born rule remains conditional rather than fully first-principles derived (\emph{A/B/C}).
