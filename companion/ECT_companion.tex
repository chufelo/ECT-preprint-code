...
\begin{abstract}
This article is a comprehensive narrative companion to the technical preprint \emph{``Euclidean Condensate Theory (ECT): Emergence of Spacetime, Quantum Mechanics, and Gravity from Spontaneous $O(4)$ Symmetry Breaking.''} ECT is presented as a common condensate architecture.  Lorentzian ordered-branch kinematics is structural, the scalar--tensor gravity ansatz is Level~B, and the present galactic and cosmological applications are separate Level~C/Open closures rather than consequences already derived from one scalar action. Time, the causal cone, the effective metric, and the split between quantum and classical dynamics arise only on the ordered branch, rather than being independent postulates.

The companion traces this single architecture across all sectors: the emergence of Lorentzian signature and the hierarchy of effective equations (Klein--Gordon, Schr\"odinger, Dirac); two energy scales ($\phi_0$ and $v_2$), a dimension-two gradient amplitude $u_0$, and a separate Level~C galactic transition length organised within one architecture; the $S_0$ action-scale route and its Level~B matching to $\hbar$, together with the arrow of time; a structural route toward the Standard Model gauge content and particle spectrum; the corrected scalar-only metric equation and a separately Open orientation-action extension; a retained five-probe cosmological consistency band combining the Hubble tension, JWST early-galaxy observations, cosmic chronometers, growth of structure, and the Integrated Sachs--Wolfe effect; a Level~C interpolation fit to 165 SPARC rotation curves, with the baryonic Tully--Fisher slope of four only as conditional algebra inside that adopted functional and with scalar matching and the environment law still Open; a reinterpretation of the cosmological-constant problem in which the direct vacuum-baseline catastrophe is blocked in the leading ordered-branch sector (Level~A) while the observed value of $\rho_\Lambda$ remains a separate Level~C infrared question; the Principle of Euclidean Stationarity and a conditional Born-rule route; decoherence and the quantum--classical boundary; a structural perspective on gravity-induced decoherence and entanglement, mediator taxonomy, and the islands/quantum-extremal-surface comparison in black-hole physics; and an explicit list of falsifiers.
...
$g_\dagger$ & Fitted acceleration scale of the adopted galactic interpolation closure; $g_\dagger\sim cH_0/(2\pi)$ is a B/C comparison ansatz, while its ECT origin and environment/history law are Open. \\
$L_{\rm gal}\equiv r_*$ & Conditional transition length $\sqrt{G_NM_{\rm bar}/g_\dagger}$ inside the adopted Level-C galactic functional; not an energy scale and not derived from the scalar ansatz. \\
...
$m_{\rm guide}$ & Imported Di\'osi--Penrose-benchmark mass guide; not an ECT-derived decoherence rate or entangling-channel crossover. \\
...
\end{equation}
contains one coordinate with a minus sign and three with a plus. This Lorentzian signature $(-,+,+,+)$ supplies the time/space distinction, hyperbolic propagation, and causal ordering.  An irreversible thermodynamic arrow requires the separate retarded influence-functional/decoherence closure. But \emph{why} does this minus sign exist?
...
The \textbf{amplitude mode} ($u$; fluctuations $\sigma=u-u_0$) is treated as heavy and short-ranged at the matched ordered point.  Its precise $u$-dependence, correlation length, and coupling to gravitational stiffness belong to the OP-grad/F7 completion; no independent Planck-length theorem is used.
...
An important structural result (the ``$P(X)$ no-orientation theorem'', proved in the preprint) shows that a purely kinetic action $P(X)$ can generate gradient condensation (nonzero $u_0$) but \emph{cannot} generate orientation stiffness --- the rigidity that resists bending of the condensate direction. Any orientation stiffness must come from higher-order terms or from the NLO sector of the EFT. A covariant $S_n[g,n,\lambda_n]$, its constraints, and its metric variation are still required before a physical orientation stress or gravitational source is obtained (Open).
...
\caption{Dimensionally separated scale inventory.  The left panel compares the two energy-dimension quantities $\phi_0$ and $v_2$; the gradient amplitude $u_0$ has units GeV$^2$ and is related only by the conditional dimensional estimate $u_0\sim\phi_0m_\sigma$.  The right panel shows the distinct length $L_{\rm gal}=r_*=\sqrt{G_NM_{\rm bar}/g^\dagger}$ inside the adopted Level-C galactic functional.  No common-axis RG flow or first-principles bridge among these objects is claimed.}
...
The \textbf{Planck-like energy scale}
$\phi_0\sim2.4\times10^{18}$\,GeV enters the ordered-medium matching
programme.  The controlled relations are
\begin{equation}
...
\end{equation}
with $M_G\sim\phi_0$ and $S_0\leftrightarrow\hbar$ retained as
Level-B matching statements rather than deductions from $\phi_0$
alone.  The gradient amplitude instead has $[u_0]={\rm GeV}^2$ and
the dimensional consistency estimate $u_0\sim\phi_0m_\sigma$; its
value is not a third energy scale.

The \textbf{electroweak energy scale} $v_2\simeq246$\,GeV is a
matched secondary scale.  The logarithm
$\mathcal I_{\rm EW}=\ln(\phi_0/v_2)\approx36.8$ organises an Open
non-polynomial scale-generation programme; none of its candidate
mechanisms has been derived from P1--P6.

The \textbf{galactic length}
$L_{\rm gal}=r_*=\sqrt{G_NM_{\rm bar}/g^\dagger}$ is conditional on
baryonic mass and the fitted acceleration $g^\dagger$ inside the
adopted Level-C functional.  For $M_{\rm bar}=10^{10}M_\odot$ and
$g^\dagger=1.2\times10^{-10}$\,m/s$^2$, it is about
$3.4$--$3.7$\,kpc.  Its scalar/ECT origin and environment law remain
Open.

Beyond the two energy scales, the same condensate pair $(\phi_0, v_2)$ naturally generates two distinguished intermediate combinations that appear in the neutrino sector: the geometric-mean scale $\sqrt{\phi_0\,v_2} \approx 2.4 \times 10^{10}$\,GeV (close to the scale of thermal leptogenesis and type-I seesaw), and the inverse-ratio Weinberg-operator scale $v_2^2/\phi_0 \approx 2.5 \times 10^{-5}$\,eV (within a few orders of magnitude of the observed neutrino mass splittings). Both combinations are the two simplest dimensionally consistent products that can be built from the pair $(\phi_0,v_2)$. They are useful ECT scale anchors for neutrino and leptogenesis model-building, but they are not first-principles derivations of the neutrino spectrum, the Dirac/Majorana character, the PMNS matrix, or the mass ordering.
...
The compact phase gives a Level-A existence result for integer winding sectors.  Turning those sectors into a distinguished physical action scale, quantum interference, or a complete discrete spectrum requires additional matching and system-specific dynamics.
...
The topological consequence is the integer winding label.  Inside the declared compact-phase closure, a candidate elementary loop action is
\begin{equation}
...
The compact phase supplies integer sectors.  A distinguished action normalisation $S_0$ belongs to the closure, and $S_0\leftrightarrow\hbar$ is a separate Level-B empirical matching step, not a topological identity. The analogy is the flux quantum $\Phi_0=h/(2e)$ in a superconductor and the Bohr--Sommerfeld condition $\oint p\,dq=nh$: in each case, single-valuedness of a periodic variable leads to quantisation, but the physical normalisation requires matching to the relevant low-energy sector.

The loop sectors partition the coherent branch into topologically distinct classes indexed by the integer $n$. Configurations in different sectors cannot be continuously deformed into each other. With an independently matched quantum measure, different sectors can acquire relative phases and interfere.  Winding topology alone does not derive the measure, arbitrary laboratory interference, or all discrete quantum labels (B/Open).
...
specified alternatives, but it does not by itself define a universal
forward/reverse trajectory-probability ratio.  Such a ratio requires a named
protocol, initial ensembles, reverse dynamics, and time-reversal convention.
No universal $P_{\rm back}/P_{\rm fwd}=e^{-\Gamma}$ law is claimed.
...
The protocol-free gamma-crossover figure is quarantined: decoherence exponents are channel-, state-, and protocol-dependent, and no universal forward/reverse law or single object-placement axis has been derived.
...
The ordered condensate organises into two complementary descriptions: the \textbf{geometric branch} (Macroscopic Physics, long-wavelength, coarse-grained, $\Gamma_{\rm loop} \gg 1$) and the \textbf{coherent branch} (Quantum Sector, short-wavelength, phase-coherent, $\Gamma_{\rm loop} \ll 1$). Both originate from the same ordered condensate action $S_{\rm ord}[u, n]$ (\emph{Level~A}). The coherent/decohered-sector distinction is structural, while the sharp crossover criterion $\Gamma_{\rm loop} \sim 1$ is a Level~B reduced-state closure estimate.
...
\textbf{Director coupling (\emph{Level~B/Open}):} The ordered-branch EFT admits the candidate operator $\mu_5\bar\Psi\gamma^A n_A\Psi$.  Its form is a Level~B operator-classification result, but $\mu_5$ has not been matched from microphysics and a constant single-species vector term may be partly removable.  Spin-precession and composition-sensitive channels therefore remain conditional; $\omega_{\rm dir}$ and $\eta\sim10^{-15}$ are target windows, not predictions.
...
\item Candidate director operator and dimensional scaling $\beta_5\sim m_f/\bar M_{\rm Pl}$ (\emph{B}); coefficient and observable bridge Open.
...
Cross-sector cone universality inside the retained native low-energy closure: $c_{\rm phase}=c_\gamma=c_{\rm GW}=c_*$; identifying $c_*$ with measured $c$ is Level-B matching.

Second law inside the Markov/ohmic influence-functional closure (Level~B): $dS_{\rm ent}/dw=N_{\rm eff}\gamma(dq/dw)^2\geq0$; only the retarded backbone is structural.

\subsection{Predictions originating in Part~I}

GW170817 consistency: $|c_{\rm GW}/c - 1| < 6\times10^{-15}$ (satisfied).

Director-coupling targets: possible spin/composition channels; quoted $\omega_5$ and $\eta\sim10^{-15}$ values are benchmark sensitivities pending microphysical matching and an observable bridge.
...
In ECT geometry is not the primitive ontology.  The effective metric is a low-energy bookkeeping object for propagation on the ordered medium.  The controlled result relevant here is narrower than the older orientation-stress narrative.
...
\begin{equation}
  ...
  \label{eq:pop_gee}
\end{equation}
where $\mathcal{T}^{(q)}_{\mu\nu}$ collects the scalar derivative and potential terms.  The factor $G_N/F$ appears only after the \emph{entire} equation is divided by $F$.  This normalization statement is derived inside the declared scalar ansatz.

Within the Level~B scalar ansatz, write $a(q)=f_{,q}/f$,
$\mathcal A=K+3f_{,q}^2/(2f)$, and
$\mathcal E=V_{,q}-2aV-(a/2)\rho_{\rm m}$.  A stable homogeneous root
$\mathcal E=0$ gives conditionally
$d\ln\bar f/d\rho_{\rm m}=a^2/(2\mathcal E_{,q})\ge0$ and
$m_{\rm loc}^2=\mathcal E_{,q}/\mathcal A$.  This is a local response,
not a finite-body screening solution.  Freezing a homogeneous reference value
is an algebraic choice inside the ansatz, not a screening mechanism or a proof
that the reference is dynamically reached.

Three objects must remain separate. M is the microscopic operator $\mu_5\bar\Psi\gamma^A n_A\Psi$; S is this Level-B scalar ansatz; and G is the independently adopted Level-C galactic functional. M$\neq$S by operator/variable content, while the physical-coordinate, source, force, and normalisation map S$\to$G is Open. The historical Lorentz-order-field appendix is therefore a mixed branch audit: its heavy-radial negative result does not select another branch, and the matter-to-Lorentz-order map remains Open.
...
The FRW background, age, Hubble-drift, and JWST diagnostic numbers quoted below use the scalar-only cosmological closure; no orientation contribution has been calculated or absorbed into them.  Practical galactic functional G is a separate Level-C fit.  The cluster script is only a local-map order/argmax audit and not a morphology closure; neither follows from Eq.~\eqref{eq:pop_gee}.  A canonical finite-body scalar charge, thin shell, Solar-System profile, and PPN projection remain Open.  The linear spin-2 route is likewise \emph{B/Open}: exact TT projection and ghost-freedom are still verification tasks.
...
\centering
\fbox{\parbox{0.86\textwidth}{\centering Superseded visual quarantined after the Round~58 semantic audit; the surrounding status-qualified text is controlling.}}
\caption{The superseded timeline asset is quarantined. Only the kinematic absence of a Lorentzian clock before ordering is structural; the ordering trajectory, $u_0$ evolution, thermal transfer, inflation replacement, and detailed cosmological history remain B/Open.}
...
Before the $O(4) \to O(3)$ transition the model uses a Euclidean arena without Lorentzian time.  The dimension-two gradient amplitude $u_0$ and the energy-dimension matching scale $\phi_0$ are distinct; no equation here sets $u_0=\phi_0$.  An ordering trajectory, branch selection, and any rapid-expansion phase require an explicit nonequilibrium completion and remain Open, so the transition is not yet a derived replacement for inflation.
...
Before ordering, the Euclidean arena has no Lorentzian light-cone horizon; that kinematic statement provides a structural reframing (Level~A).  Demonstrating that the observed CMB uniformity is dynamically inherited by the selected Lorentzian branch requires the ordering and thermal history and remains Level~B/Open.
...
ECT addresses this through two complementary mechanisms, co-directed inside the shared diagnostic parametrisation, while their common microscopic origin remains Open:

First, the evolving $G_{\rm eff}(z) > G_N$ at high~$z$ means gravitational collapse was \emph{more efficient} in the early universe.

Second, the adopted scalar-only cosmological diagnostic closure includes an additional effective response term beyond the uniform $G_{\rm eff}(z)$ drift.  This term is a specified diagnostic input, not a computed orientation stress.

An important and initially counterintuitive point is that ECT does \emph{not} solve the JWST tension by simply giving early galaxies more time. In the diagnostic layer used here, the ordered Lorentzian branch is actually somewhat younger than in the corresponding $\Lambda$CDM reference, so the formation-time budget at $z \sim 10$ is reduced rather than enlarged. The reason the JWST channel still moves in the same direction as the Hubble channel is different: within the scalar-only diagnostic closure, the stronger fitted early response and its additional effective term accelerate collapse enough to overcompensate for the reduced time budget. The co-directed Hubble--JWST movement is a non-trivial consistency feature of the shared diagnostic parametrisation.  The added JWST response term and the Hubble mapping have not been derived from one microscopic condensate mechanism.
...
The galactic interpolation is specified and documented as a phenomenological fit closure; it is not derived from the corrected scalar--tensor action (\emph{Level~C}).  Its amplitude coordinate $\phi_u=\beta_\phi^{-1}\ln(u/u_\infty)$ is distinct from the anisotropy coordinate $\psi_\chi=\ln(\chi/\chi_{\rm vac})$.  The provisional one-loop result $K_\chi(\psi_\chi)\propto e^{-\psi_\chi/2}$ cannot determine the $\phi_u$ coefficients or derive the AQUAL/interpolation branch without the missing $u\leftrightarrow\chi$ map.  The high-acceleration limit is Newtonian only algebraically inside the specified closure; finite-body screening, body charge, and Solar-System reachability remain Open.  The interpolation is:
\begin{equation}
  g(R) = \tfrac{1}{2}\bigl(g_N + \sqrt{g_N^2 + 4\,g_N\,g^\dagger}\bigr),
  \label{eq:pop_interp}
\end{equation}
with the matched reference $g^\dagger_0\sim cH_0/(2\pi)\approx1.04\times10^{-10}$\,m/s$^2$; per-galaxy $g^\dagger_{\rm eff}$ values are Level-C fit outputs.  Inside this adopted closure, the deep-field limit ($g_N \ll g^\dagger$) is $g \approx \sqrt{g_N g^\dagger}$ and the strong-field limit is $g \approx g_N$.  These are algebraic properties of the interpolation, not a derivation of the physical branch or of finite-body screening.  Separately, the displayed force-law dimensionality curve assumes the illustrative point-source alternative $\mu_{\rm dim}=x/\sqrt{1+x^2}$, not practical G.  Extended-galaxy extraction requires the measured local baryonic slope $p_N(r)$ and has not been performed (Level~C algebra / Open test).
...
This phenomenological closure has been fitted to \textbf{165 galaxies from the SPARC database} (Fig.~\ref{fig:pop_mw}, \ref{fig:pop_sparc}).  The sample medians are $\chi_r^2\approx2.7$ for fixed stellar mass-to-light ratio and $\approx1.4$ when it is free, versus the quoted MOND comparator $\approx3.1$ on comparable parameter footing.  The values $2.95$ and $6.55$ belong only to the displayed Milky-Way benchmark and must not be attributed to the 165-galaxy sample.  All are Level~C fit diagnostics, not evidence for scalar matching or an environment law.
...
\caption{The baryonic Tully--Fisher relation (BTFR) from the SPARC sample. Within the adopted galactic interpolation closure, the deep-branch algebra gives slope~4 (solid line): $v_{\rm flat}^4 = G_N M_{\rm bar}\,g^\dagger$. (\emph{Slope: Level~B/C conditional on the closure; fitted $g^\dagger$ value: Level~C.})}
\label{fig:pop_btfr}
\end{figure}

The BTFR --- $M_{\rm bar} \propto v_{\rm flat}^4$ --- is one of the tightest empirical correlations in extragalactic astronomy (Fig.~\ref{fig:pop_btfr}). In $\Lambda$CDM, reproducing it requires careful coordination between baryons and halo structure.  In the adopted ECT galactic interpolation closure, the BTFR slope is an exact algebraic consequence of the assumed deep branch:
\[
v_{\rm flat}^4 = G_N M_{\rm bar}\,g^\dagger .
\]
The slope~4 is therefore not fitted \emph{inside} that closure, but selection of the physical branch and the fitted normalization remain Level~B/C rather than a bare-theory prediction.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{ect_rar_new_6panel_bw.pdf}
\caption{Radial acceleration relation (RAR) diagnostics from the SPARC sample.  The adopted closure is compared with the matched reference $g^\dagger_0\approx1.04\times10^{-10}$\,m/s$^2$ and the empirical MOND/RAR scale $a_0\approx1.2\times10^{-10}$\,m/s$^2$. Per-galaxy $g^\dagger_{\rm eff}$ values are Level~C fit outputs; the RAR is not an independent test of the same fitted closure.}
\label{fig:pop_rar}
\end{figure}

The RAR (Fig.~\ref{fig:pop_rar}), discovered by McGaugh et al.\ (2016), is reproduced by the adopted interpolation closure~\eqref{eq:pop_interp}; it is not an independent theorem of the bare postulates.  An environment/history factor $\Xi$ may be tested exploratorily, but neither its sign nor its amplitude is derived (\emph{Level~C/Open}).
...
\paragraph{UDG inverse diagnostic after the Wolf-normalisation audit.}
At the Wolf radius $R_{1/2}=4R_e/3$, the isotropic proxy uses
$M_{\rm dyn,1/2}=3\sigma_{\rm los}^2R_{1/2}/G$ and
$M_{\rm bar,1/2}=M_\star/2$.  Defining
$\mathcal R_{\rm dyn}=g_{\rm obs}/g_N$, inversion of practical G gives
\[
\Xi_{\rm alg}=\mathcal R_{\rm dyn}(\mathcal R_{\rm dyn}-1)
\frac{g_N}{g^\dagger_0}.
\]
This is an unconstrained algebraic diagnostic, not a predicted environment
factor.  It is physical only for $\Xi_{\rm alg}\ge0$ (hence
$\mathcal R_{\rm dyn}\ge1$).  The central
$(\mathcal R_{\rm dyn},\Xi_{\rm alg})$ values are
$(0.787,-0.00369)$ for DF4, $(1.229,0.00516)$ for FCC~224,
$(10.261,0.892)$ for NGC~5846-UDG1, and $(31.729,4.981)$ for
Dragonfly~44; DF2's quoted dispersion bracket gives
$\Xi_{\rm alg}=0.031$--$0.423$.  Thus DF4 has no admissible
$\Xi\ge0$ solution at its central proxy inputs.  A boundary-aware Jeans
likelihood remains Open.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig_udg_regime_diagram.pdf}
\caption{Direct acceleration-residual view for the UDG reference objects.
The comparison uses $g_{\rm obs}=3\sigma^2/R_{1/2}$ and the adopted
Level-C closure; it assigns no microscopic scalar coordinate and is not a
screening result.}
...
\caption{Corrected $\Xi_{\rm alg}$ inverse diagnostic.  The DF4 boundary
marker denotes no physical nonnegative-$\Xi$ solution at the central inputs.
Quoted-dispersion ranges are boundary checks, not a common 95\% nuisance
posterior.  All results are Level~C proxy algebra; the environment law is
Open.}
\label{fig:pop_udg}
\end{figure}

The earlier logarithmic uncertainty band and notation that treated the
algebraic inverse as an already inferred physical factor are withdrawn.  These objects test
the adopted closure and its future environment/history completion; they do
not by themselves falsify ECT or establish a density-only $\Xi$ law.

\FloatBarrier\section{Cluster projected-proxy audit}
\label{sec:pop_clusters}

The current script applies the strictly local map
$\Sigma_{\rm cl}=\Sigma_b\nu_{\rm cl}(2\pi G_N\Sigma_b/g^\dagger_{\rm cl})$
to hand-set Gaussian baryonic maps.  For $D=\sqrt{1+4/y^2}$,
\[
...
\]
Thus the transform preserves every positive-pixel ordering and compresses
contrast.  The BCG/gas peak class is inherited from the input map; the four
systems are implementation sanity checks, not reproduced morphology,
lensing inference, or evidence for ECT.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{fig_bullet_main.png}
\caption{Bullet-cluster local-map audit.  The proxy peak must coincide with
the hand-set baryonic-map peak by exact monotonicity.  Normalisation and
orientation entries are hand-set/external sensitivities, not observed mass
ratios.}
...
\caption{Four-system order/argmax-preservation audit with toy Gaussian
baryonic inputs.  The plotted $A_0^{\rm proxy}$ and
$A_{\rm fin}^{\rm proxy}$ are deterministic hand-set normalisation
sensitivities, not observational amplitudes.}
\label{fig:pop_clusters}
\end{figure}

A genuine cluster test requires independently constrained baryonic maps, an
action-matched physical (possibly nonlocal) source/lensing response, and a
prespecified external likelihood.  All remain Open.
...
ECT contains candidate ingredients compatible with the three Sakharov conditions only inside declared effective closures (Level~B): an ordering transition, a candidate $CP$-odd/director sector, and topology-changing channels.  Their simultaneous physical realisation and an ECT-native baryogenesis calculation are Open.  The quoted $\eta_B\sim9\times10^{-10}$ is an external/benchmark closure value, not an ECT prediction.
...
\item JWST diagnostic response via enhanced $G_{\rm eff}$ plus an adopted additional closure term (\emph{B/C}; orientation attribution Open).
...
\item RAR fit with $g^\dagger \approx cH_0/(2\pi)$ (\emph{C}); the physical matching and environment law are Open.
\item Cluster local-map order/argmax identity (Level~C algebra); physical lensing and morphology test Open.
\item Baryogenesis: Sakharov-compatible ingredients exist in declared closures (\emph{B}); an ECT-native $\eta_B$ derivation is Open.
...
\begin{itemize}
\item BTFR slope = 4 conditional on the adopted galactic closure (\emph{B/C}); fitted $g^\dagger \sim cH_0/(2\pi)$ (\emph{C}).
\item Director operator form (B), with $\eta\sim10^{-15}$ only a C/Open target sensitivity; dark energy $w_0\approx-0.83$ is a calibrated closure benchmark (C).
\item Exploratory environment/history law $\Xi$ (\emph{Open}); graviton polarisation: two TT modes in the candidate linear closure (\emph{B/Open}).
\item CMB quadrupole deficit (\emph{B/Open}).
\end{itemize}
Full list with explanations is given in Part~V, Section~\ref{sec:pop_predictions}.

The stale cosmology-prediction image is quarantined because its embedded $\varepsilon$, $\Delta H_0$, and $w_0$ labels predate the retained-band/shared-diagnostic status audit.
...
The status-separated derivation logic of Part~II is shown in Fig.~\ref{fig:pop_partII_logic}; the superseded cosmology-prediction visual is quarantined, so the preceding status-qualified text controls.
...
The stale ECT-versus-QM image is quarantined because its embedded Born and measurement labels exceed the conditional C1--C3/Open status.
...
whereas interacting states, Born weights without C1--C3, detector updates, and
...
and a detector update theorem are not established.  Quadratic weights require
the separate C1--C3/RP--OS/Gleason assumptions; this is a B/Open route rather
than an explanation from bare P1--P6.
...
\item[ECT (physical picture).] Both branches arise from the same ordered-condensate architecture.  The controlled long-wavelength equation used here is scalar-only; a geometric orientation stress is a separate Open $S_n[g,n,\lambda_n]$ extension.  The shorter-wavelength coherent sector is read wave-mechanically through the Klein--Gordon and Schr\"odinger-type limits.
...
\item a programme for deriving an ECT gravitational record channel; current Di\'osi--Penrose values are imported benchmarks, while the physical ECT vertex, state, and noise kernel remain Open;
...
\textbf{Interference route} (\emph{B/Open}): compact phase and matched path-integral weighting give the usual constructive/destructive combination. Topology alone does not derive the full quantum measure or arbitrary laboratory patterns.
...
\section{PES-R: status-separated persistence filters}
\label{sec:pop_pes}

PES-R is retained as an organising inventory, not a universal selection
theorem.  Action stationarity belongs to the Euclidean variational problem;
pairwise record formation is described by a protocol- and state-dependent
distinguishability exponent $\Phi_{ab}(T)$; finite-time stability is probed by
sector-specific singular values or decay widths $\gamma_a$; and entropy
production $\Sigma$ belongs to a named forward/reverse protocol.  A declared
translation susceptibility $\Gamma_{\rm tr}[q;\eta]$ is a diagnostic, not a
mean-equation functional.

No universal quietness ratio, additive action--decoherence functional, or
necessary-and-sufficient intersection theorem has been derived. PES-R does
not derive Born weights, energy eigenstates, the least-action principle,
discrete spectra, or a universal quantum--classical boundary. The technical
preprint still requires the separately reviewed PES-R repair before any such
owner block can be treated as current.
...
entanglement while the reduced state remains coherent
($\Gamma_{\rm loop}\ll1$).  Decoherence is a distinct process: uncontrolled
record channels are traced out and off-diagonal coherences are suppressed.
It is therefore not a prerequisite for interaction, and no universal
$\Gamma_{\rm loop}\sim1$ interaction threshold exists.  The crossover is only
a Level-B estimate for irreversible event-like record formation in a specified
reduced model.
...
The influence-functional framework in Fig.~\ref{fig:pop_qubit} provides a common mathematical language for specified environmental channels and for a future gravity-related record channel.  For gravity, however, the physical vertex, state, and noise kernel have not yet been identified, so no absolute ECT rate follows from the shared formalism.
...
\textbf{Gravity-related decoherence}: current Di\'osi--Penrose arithmetic is an imported benchmark, not an ECT-derived rate.  The projected linear orientation density vertex is missing, the scalar trace vertex is only parametric, and the relevant ECT state/noise kernel is Open/Not Identifiable.

\paragraph{DP-02 spectral red-team.}
For the tested minimal free-vacuum scalar channel,
$J_{\phi}(\omega)\propto\omega^3$.  For the tested equal-mass Gaussian
composite channel, the vacuum spectrum instead scales as
$J_{\mathcal O}(\omega)\propto\omega^9$ and its finite-time tail/saturation
has envelope $T^{-8}$.  In the thermal regime $\omega\ll ck\ll T_b$, the
same composite benchmark scales as $J_{\mathcal O}^{\rm th}\propto T_b^5/k$
(with coefficient $2\pi^3/[15Z^2u_0^4c^8]$ in the declared convention).
These tested closures therefore do not produce a universal Diósi--Penrose
$1/k^2$ infrared kernel.  An alternative ECT state, pole, physical vertex,
and normalisation remain Open.
...
\textbf{Recoherence.} Partial restoration of coherence is possible in specified echo/control protocols.  The decoherence exponent controls off-diagonal suppression, but no universal forward/reverse probability law follows from it without a fully specified protocol and dynamics.
...
ECT takes a candidate architectural route (Level~B/Open).  The full
condensate is postulated not to destroy information fundamentally, but a full
interacting OS/unitarity theorem has not been established.  Reduced coherent
...
state, vertex, kernel, and reduction map and remain Open.  The stale two-level
ontology figure is quarantined because it displayed these hypotheses as facts.
...
\textbf{Imported benchmark and crossover audit.}
Equating the imported Di\'osi--Penrose estimate
$\tau_{\rm DP}^{\rm bench}\sim\hbar R/(G_Nm^2)$ to a declared
coherence time defines only the benchmark guide
\[
 m_{\rm guide}^{\rm bench}
 \sim \sqrt{\hbar R/(G_N\tau_{\rm exp})}.
\]
It is not a scalar-sector or entangling-channel ECT prediction.  A
collisional/D--P comparison additionally requires a declared gas model,
$n_{\rm gas}=P/(k_BT)$, a thermal-speed convention, cross section, geometry,
and coherence protocol.  The legacy numerical crossover rows labelled only
by $(P,T)$ are not reproducible under their stated ideal-gas N$_2$
convention and are not carried forward as verified benchmarks.  A completed
ECT crossover would further require the physical vertex, state/noise kernel,
and finite-body charge, all of which remain Open/Not Identifiable.
...
a finite-body charge, or an absolute decoherence rate.  The values obtained
from $\tau^{\rm grav}_{\rm dec}\sim\hbar R/(G_N m^2)$ are therefore imported
Di\'osi--Penrose benchmarks.  The associated
$m_{\rm guide}\sim\sqrt{\hbar R/(G_N\tau_{\rm exp})}$ is only a benchmark
mass guide, not an ECT-derived crossover.
...
record channel is therefore Open/Not Identifiable; the Di\'osi--Penrose
numbers remain Level~C external comparators.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_mediator_channels.pdf}
\caption{Operational taxonomy for candidate gravity-related record channels.  The scalar trace vertex is parametric, while its state/noise kernel and finite-body charge are Open.  The orientation/tensor route lacks a derived density projector and a specified state.  The displayed Di\'{o}si--Penrose timescale $\tau_{\rm dec}^{\rm grav}\sim\hbar R/(G_N m^2)$ and $m_{\rm guide}\sim\sqrt{\hbar R/(G_N\tau_{\rm exp})}$ are imported benchmarks, not ECT-derived rates or crossover predictions. (\emph{Level~C/Open.})}
...
\caption{Candidate Level-B critical-shell picture. A shell/interface may replace the classical horizon inside this closure, while its geometry, correlations, and Euclidean-core continuation remain Open. No fundamental information destruction is an architecture-level postulate; a constructive outgoing-correlation recovery is not derived.}
\label{fig:pop_bh}
\end{figure}
...
\item Di\'osi--Penrose benchmark retained; the ECT gravitational record channel is Open/Not Identifiable.
...
BH entropy target: $S_{\rm BH}=A/(4\ell_{\rm Pl}^2)$; the current ECT shell result establishes area scaling only, with the coefficient $1/4$ still Open.
...
\textbf{MOND (Milgrom, 1983):} The critical acceleration $g^\dagger$ is introduced as a universal free constant. In practical functional G the corresponding scale is fitted and compared with $cH_0/(2\pi)$; its microscopic origin and any environment/history law remain Open. The characteristic acceleration and the fitted interpolation are Level~C inputs; the BTFR slope is conditional B/C algebra inside that functional, while a universal environment law remains Open.
...
\textbf{Common pre-geometric architecture of interactions:} Gravity, the Abelian gauge route, the candidate weak sector, the colour sector, and the candidate director EFT operator are organised within the same condensate architecture, but with different logical status; the director coefficient and observable magnitudes remain Open. Gravity arises from the ordered condensate geometry. The compact phase fibre gives a strong Abelian bundle route. The weak sector has a candidate orientation/Hopf-locking route, but the full local chiral $SU(2)_L\times U(1)_Y$ Standard-Model realisation remains open. The colour sector requires extended internal structure. This is broader than conventional Grand Unification, which does not include gravity in the same condensate framework. The weak-sector parity asymmetry acquires a structural route from the condensate orientation, but the maximally parity-violating Standard-Model pattern remains an open derivation problem.

\textbf{Director coupling:} The stated EFT basis admits a lowest-dimension fermion--condensate operator (B).  Observable spin/composition signals require non-universality, gradients, curvature, or relative multi-sector measurements; their magnitudes remain Open rather than derived predictions.
...
Pauli exclusion principle & Postulate & Free-kernel antisymmetric OS consistency is mathematical A; application to a physical ECT spinor/CAR sector is B/Open & A-math / B--Open \\
...
\midrule
Metric/gravity route & Einstein equations postulated in GR & Scalar-only metric equation holds inside its Level-B ansatz; the linear spin-2 route is B/Open, while covariant $S_n$, physical orientation stress, TT projection, and nonlinear completion remain Open & B/Open \\
...
Particle dark matter (galactic fit channel) & Postulated halo & Specified interpolation fits rotation curves (C); BTFR slope 4 is conditional algebra inside that closure (B/C) & C / B--C \\
...
Measurement problem & Open problem & Reduced-sector decoherence can suppress interference; conditional Born-type weights require C1--C3, while pointer-basis, single-outcome, and update theorems remain Open & Open \\
\end{longtable}
...
$c_{\rm phase}=c_\gamma=c_{\rm GW}=c_*$ within the retained native low-energy closure; $c_*=c$ is an empirical matching step.
...
$S_{\rm BH}=A/(4\ell_{\rm Pl}^2)$; ECT currently establishes area scaling, while the coefficient $1/4$ remains Open.
...
\item \textbf{$g^\dagger \sim cH_0/(2\pi)$} (\emph{B/C structural matching ansatz}). Its empirical comparison is meaningful, but the physical matching from one ECT coordinate remains Open.

\item \textbf{Director-coupling WEP window} (\emph{C/Open}).  $\eta\sim10^{-15}$ is a target sensitivity, not a derived ECT value; the microphysical coefficient and observable multi-species bridge remain Open.
...
\item \textbf{Exploratory environment/history law $\Xi$ for $g^\dagger_{\rm eff}$} (\emph{Open}). No sign or amplitude is predicted. A null result rejects only the declared $\Xi$ ansatz; a correlation would motivate, but would not derive, an ECT mechanism.
...
and an explicitly external anisotropic proxy $\mathcal X^{\rm cand}_{\mu\nu}[n]$ leave a detectable
...
remains future work.
Any additional slip contribution from $\mathcal X^{\rm cand}_{\mu\nu}[n]$ is
...
\item \textbf{The dark matter problem.} No particle dark matter has been detected despite decades of searches.  The specified ECT interpolation fits the galactic mass-discrepancy phenomenology without a particle-halo carrier in that fit channel (\emph{C}); its source/force map is not derived, and this does not exclude hidden-sector particles.
...
\item \textbf{JWST early galaxies.} A scalar-only diagnostic closure with larger fitted $G_{\rm eff}$ and an additional effective response term can accelerate structure formation (\emph{C}); no orientation contribution is computed.

\item \textbf{Baryon asymmetry.} Sakharov-compatible ingredients are available only within declared closures (B); $\eta_B\sim9\times10^{-10}$ is an external benchmark and the ECT-native derivation is Open.
...
stress remains Open.  The late-time cosmological band is a calibrated
diagnostic closure.  The 165-galaxy SPARC fit has median
$\chi_r^2\approx2.7$ for fixed $M/L$ and $\approx1.4$ for free $M/L$,
versus the quoted MOND comparator $\approx3.1$ on comparable footing
(Level~C); its scalar/ECT source map and any $\Xi$ law are Open.  Decoherence,
PES, black-hole area scaling and Page-curve-like shell behaviour, and the
boundary-sensitive Casimir response retain their individual B/Open ceilings;
...
reduced model.  The imported $38$~ms Diósi--Penrose number is not an ECT rate,
and recoherence remains protocol dependent.
...
Why $3+1$? & Given P1, $O(4)\to O(3)$ supplies one temporal seed and three spatial directions; the origin of four dimensions remains OP0 & A conditional on P1 & Structural \\
Primary monopoles & $\pi_2(S^3)=0$ excludes primary ordering monopoles; later gauge-sector monopoles are completion dependent & A/Open & Sector limited \\
Vacuum stability & Tree/local and primary-amplitude one-loop checks hold in their declared closures; global/metastable completion remains Open & B/Open & Closure limited \\
...
Massless spin-2 route & Linear effective-metric route is conditional; covariant orientation action, TT projection, and nonlinear completion remain Open & B/Open & Candidate \\
Gravitational record channel & $38$~ms for micron nanospheres is an imported Diósi--Penrose benchmark; the ECT vertex/state/noise kernel is Open & C/Open & Not ECT-specific \\
...
Dark-energy benchmark & $w_0\approx-0.83$ belongs to a calibrated thawing closure; $w=-1$ would reject that closure, not all ECT & C/B-closure & Diagnostic \\
Director/WEP target & Operator classification is B; $\eta\sim10^{-15}$ is only a C/Open sensitivity target pending coefficient and observable bridge & B/C/Open & Target only \\
\end{longtable}
\normalsize
