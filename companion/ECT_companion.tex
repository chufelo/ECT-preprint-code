...
\caption{The ECT Hubble-tension alleviation mechanism. The evolving effective gravitational coupling $G_{\rm eff}(z) > G_N$ at high redshift shifts the late-time $H_0$ scale upward; at the retained joint-band central value $\varepsilon \approx 0.033$ the shift is compatible with the full $\sim 5$\,km/s/Mpc Planck-SH0ES gap at the baseline-background mapping level (an accompanying age-viability note attends this central value). (\emph{Sign: Level~A structural prediction; magnitude: Level~B diagnostic-layer indicator.})}
...
This benchmark is not obviously in tension with the broad range of
published ISW amplitude measurements, but the comparison remains
approximate: the present treatment uses an overlap proxy rather
than a full $C_\ell^{Tg}$ observable, and a non-Limber Boltzmann-level
calculation with realistic tracer windows remains future work.
...
The full derivation, diagnostic figures, benchmark values, and
