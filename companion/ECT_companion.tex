...
\medskip
\noindent\textbf{Principal symbols.}\\[2pt]
\begin{tabular}{@{}p{2.6cm}p{12.6cm}@{}}
...
\caption{The ECT Hubble-tension alleviation mechanism. The evolving effective gravitational coupling $G_{\rm eff}(z) > G_N$ at high redshift shifts the late-time $H_0$ scale upward; at the retained joint-band central value $\varepsilon \approx 0.033$ the shift is compatible with an upward $H_0$ shift of the observed Planck--SH0ES size, $\sim 5$\,km/s/Mpc, at the baseline-background mapping level (an accompanying age-viability note attends this central value). (\emph{Sign: Level~A structural prediction; magnitude: Level~B diagnostic-layer indicator.})}
