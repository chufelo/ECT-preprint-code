#!/usr/bin/env python3
"""Build the R153 reader-facing coherence-retention regime figure.
...
SCRIPT_VERSION = "R153 coherence-regime geometry successor v1"
FIXED_TIME = dt.datetime(2026, 7, 25, 12, 0, 0, tzinfo=dt.timezone.utc)
X_MIN = 1.0e-3
X_LEFT_GUIDE = 1.0e-1
X_RIGHT_GUIDE = 3.0
X_MAX = 3.0e1
...
def build(outdir: Path) -> None:
    ...
    fig = plt.figure(figsize=(6.25, 4.65), constrained_layout=False)
    grid = fig.add_gridspec(
        nrows=2,
        ncols=1,
        height_ratios=(1.05, 3.95),
        hspace=0.065,
        left=0.105,
        right=0.985,
        bottom=0.195,
    ...
    pdf_path = outdir / "fig_coherence_regimes_r153.pdf"
    png_path = outdir / "fig_coherence_regimes_r153.png"
    pdf_metadata = {
        "Title": "Protocol-dependent coherence-retention regimes",
        "Author": "ECT R153 reproducibility pipeline",
        "Subject": "Pure-dephasing model visual guide; not a universal boundary",
        "Keywords": "ECT R153 coherence dephasing visibility protocol guard",
    ...
    runtime = {
        ...
        "outputs": {
            "fig_coherence_regimes_r153.pdf": sha256(pdf_path),
            "fig_coherence_regimes_r153.png": sha256(png_path),
            "previews": preview_hashes,
        },
    }
    (outdir / "R153_COHERENCE_RUNTIME_v1.json").write_text(
