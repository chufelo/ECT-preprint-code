...
\large\textcolor{red}{[SUPERSEDED INTERNAL DRAFT --- see note below]}}
\author{ECT working notes --- v4 consolidated (internal, historical)}
\date{2026-04-20}
...
\begin{center}
\fbox{\parbox{0.92\linewidth}{\footnotesize
\textbf{Status notice (2026-04-21).}
This document is \emph{internal historical working notes}.  It does
\textbf{not} reflect the final published position of the UDG stress
test in the main preprint.  Specifically, earlier versions of this
document at various times contained claims of the form
``DF4, FCC~224, and DF2 are predicted to be Regime~III objects'',
``no new phenomenology is introduced'',
``all three predictions match observation'', and
``no resolution of OP2/OP3 is required''
--- all of those claims are \emph{withdrawn} and are \emph{not}
part of the published ECT preprint.  The current published position
is that the dark-matter-deficient UDGs constitute a \emph{stress
test} of the current Level-B galactic closure, not a solved
consequence of ECT.  For the authoritative published formulation
see Section~16.5 (``Dark-matter-deficient ultra-diffuse galaxies
as a stress test of the current closure'') and
Appendix~AR (``Diagnostic stress test of the current Level-B
galactic closure: dark-matter-deficient ultra-diffuse galaxies'')
of the main ECT preprint.
}}
\end{center}
