...
ect_epochs = [
    (XMIN,         LOG_PT,          'Euclidean phase\nO(4)-symmetric\nno time, $u_0=0$', G['undefined']),
    (LOG_PT,       LOG_ECT_PT_END,  'PT\nO(4)$\\to$O(3)',                                  G['planck']),
    (LOG_ECT_PT_END, LOG_END_INF,   'Ordering\ntransition',                    G['inflation']),
...
notes = [
    (XMIN+1,          r'$O(4)$-symm.'),
    (LOG_PT+0.2,       r'$\alpha>1$, time emerges, $c_*=1/\sqrt{\alpha-1}$, $u_0{\sim}\bar{M}_{\rm Pl}$ frozen'),
...
# ── Plot ──────────────────────────────────────────────────────────────────
YMAX = 1.75
ax3.plot(t, v0_A,  '-',  color='black', lw=2.2, label=r'$u_0(t)/u_0(\infty)$ — Planck condensate (Scen. A)')
ax3.plot(t, v0_B,  ':',  color='black', lw=1.8, label=r'$u_0(t)/u_0(\infty)$ — Planck condensate (Scen. B)')
...
ax3.plot(t, G_eff_B, '-', color='0.30', lw=1.4, label=r'$G_{\rm eff}/G$ (Scen. B,  $\propto u_0^{-2}\to\infty$)')
