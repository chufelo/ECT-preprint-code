...
for ax, (data, title, cmap, vmin, vmax, fmt) in zip(axes2.ravel(), panels):
    ...
    ax.set_xlabel(r"$\omega_0$"); ax.set_ylabel(r"$\phi_{\rm bg,0}$")
...
fig2.suptitle(r"Conditional background scan: $\beta=0.8$, $\widehat{\mu}_{\rm bg}=1.5$, "
              r"$A_2=\widehat{\mu}_{\rm bg}^{2}/(6\beta^2)$, $A_3=A_4=0$", fontsize=11, fontweight='bold')
...
ax.text(0.88, 0.66, r"$\phi_{\rm bg} \to 0$"+"\n(reference limit)", ha='center', fontsize=9, color='#333333')
...
ann = [(0.11, 0.17, "Lorentzian branch\nemerges"),
       ...
       (0.85, 0.17, "Benchmark truncation\nconditional (not a screening derivation)")]
for x, y, lab in ann:
...
fig5.suptitle("ECT JWST anchor case budget — Hubble-priority working point "
              r"($\omega_0=30$, $\phi_{\rm bg,0}=-0.10$)",
