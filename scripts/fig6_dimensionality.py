#!/usr/bin/env python3
"""
Illustrative force-law dimensionality benchmark d_force(r).
...
The displayed formula d_force(x) = 1 + 2(1+x²)/(2+x²) belongs to
the illustrative alternative interpolation mu_dim(x)=x/sqrt(1+x²).
It is not the adopted SPARC/G law and is retained only as a Level-C
benchmark for synthetic point-source masses and the analytic relation.
"""
...
gal_panel_a = [
    (r'Dwarf-mass point source ($10^8\,M_\odot$)',             1e8*M_sun,  '--', '0.55', 1.2),
    (r'MW-mass point source ($5{\times}10^{10}\,M_\odot$)',     5e10*M_sun, '-',  '0.15', 2.0),
    (r'Giant-mass point source ($5{\times}10^{11}\,M_\odot$)',  5e11*M_sun, '-.', '0.35', 1.2),
]
...
ax.annotate('illustrative $\\mu_{\\rm dim}$ branch:' + '\n$g\\propto 1/r$\n$\\Rightarrow v_{\\rm flat}$',
...
ax.annotate(fr'$r_*\ (g_N=g^\dagger)\approx {rs_MW:.0f}$ kpc', xy=(rs_MW/1000, d_rs_MW), xytext=(rs_MW/1000*5, 2.6),
...
# ── Panel (b): analytic mu_dim relation (no data overlay) ──────
ax = ax2
x_arr = np.logspace(-2, 3, 500)
d_arr = d_force_analytic(x_arr)

ax.plot(x_arr, d_arr, '-', color='0.15', lw=2.0, zorder=2, label=r'Illustrative $\mu_{\rm dim}$ relation (Level C)')
ax.axhline(3, color='0.82', ls=':', lw=0.7); ax.axhline(2, color='0.82', ls=':', lw=0.7)
# No x=1 transition marker: r_* below is defined instead by g_N=g^dagger.

# Generated profile points would be tautological because x and d are
# both computed from this same mu_dim law; they are deliberately omitted.
...
ax.text(0.02, 2.05, '$d\\to 2$ (illustrative $\\mu_{\\rm dim}$)', fontsize=7.5, color='0.40', ha='center')
...
ax.set_xlabel(r'$x = g_{\rm obs}/g^\dagger$', fontsize=10)
...
fig.text(0.5,0.008,r'Synthetic benchmark inputs: $g^\dagger=1.2\times10^{-10}\,\mathrm{m\,s^{-2}}$ and idealized point-source masses; no observational data overlay; faded isolation cutoffs are display-only.',ha='center',fontsize=7.2,style='italic')
