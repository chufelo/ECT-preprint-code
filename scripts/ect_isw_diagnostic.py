...
print(f"Baseline: A_proxy={A_base:.4f}, source-power diag: +{power_diag:.1f}%")
print(f"deta=0.02: {A_02:.4f}; deta=0.05: {A_05:.4f}")
...
# ==== FIGURE 1: 4-panel diagnostic (GRAYSCALE) ====
...
ax.legend(fontsize=9,loc='upper right',framealpha=.9); ax.set_title('(b) ISW source kernel')
ax=axes[1,0]
...
ax.annotate('benchmark',xy=(1.,0.),xytext=(2.,2.5),fontsize=9,
            arrowprops=dict(arrowstyle='->'),bbox=dict(boxstyle='round',fc='white',ec='gray'))
ax.set_xlabel(r'$\varepsilon$ [%]'); ax.set_ylabel(r'$\delta\eta_{\max}$ [%] (illustrative ansatz)')
ax.set_title(r'(d) Overlap proxy $A(\varepsilon,\delta\eta)$')
plt.tight_layout()
plt.savefig(OUT/"ect_isw_diagnostic_v3.png",dpi=300,bbox_inches='tight'); print("Fig 1 saved.")

# ==== FIGURE 2: Proxy vs published amplitudes (GRAYSCALE) ====
fig,axes=plt.subplots(1,2,figsize=(13,5))
ax=axes[0]
onames=list(obs.keys()); ovals=[obs[n][0] for n in onames]; oerrs=[obs[n][1] for n in onames]
ax.errorbar(ovals,range(len(onames)),xerr=oerrs,fmt='ks',ms=7,capsize=5,capthick=1.5,lw=1.5,zorder=5,label='Published amplitudes')
ax.axvline(1.,color='gray',ls=':',lw=1.5,label=r'$\Lambda$CDM')
ax.axvline(A_base,color='k',ls='-',lw=2,alpha=.8,label=f'ECT baseline proxy ({A_base:.3f})')
ax.axvline(A_02,color='k',ls='--',lw=1.5,alpha=.7,label=f'illustr. deta=0.02 ({A_02:.3f})')
ax.axvline(A_05,color='k',ls=':',lw=2,alpha=.7,label=f'illustr. deta=0.05 ({A_05:.3f})')
...
ax.annotate('eps=0.01 from Hubble benchmark\nproxy comparison only\nnot fitted to ISW',
            xy=(A_base+.02,2.3),fontsize=8,bbox=dict(boxstyle='round',fc='white',ec='gray',alpha=.9))
ax=axes[1]
Ascan=[]
for ep in escan:
    _,Dv,dDv=solve_growth(lambda a,e=ep:a**(-2.*e))
    Div=interp1d(ag,Dv,'cubic')
    Kv=np.array([isw_kernel(a,Div,interp1d(ag,dDv,'cubic'),lambda a,e=ep:a**(-2.*e),lambda a:1.) for a in a_arr])
    Ascan.append(overlap_ratio(Kv,Div(a_arr),Kl,Di_l(a_arr)))
Ascan=np.array(Ascan); bv,be=0.96,0.30
...
ax.axhline(bv,color='k',ls=':',lw=1,alpha=.5); ax.axhline(1.,color='gray',ls=':',lw=1)
...
ax.text(4.,.9,'representative\nfuture precision',fontsize=7.5,ha='center',color='gray',style='italic')
ax.set_xlabel(r'$\varepsilon$ [%]'); ax.set_ylabel('Overlap proxy amplitude')
...
plt.tight_layout()
plt.savefig(OUT/"ect_isw_vs_obs_v3.png",dpi=300,bbox_inches='tight'); print("Fig 2 saved.")
print("Done.")
