...
Proposal-only cluster peak-preservation sanity check using an
independent alternative Level-C interpolation benchmark for four
merging systems: Bullet, MACS J0025, El Gordo, and Abell 520.  Its
nu_cl(y)=sqrt((1+sqrt(1+4/y^2))/2) is not the SPARC/G interpolation
and no transfer from the practical galactic closure is claimed.  It
is the inverse-boost representation of the same alternative
mu_dim(x)=x/sqrt(1+x^2) benchmark used by the dimensionality asset.

The benchmark-weighted surface-density maps are phenomenological
proxies built from hand-set toy Gaussian masses, widths, and
offsets; they are not source-reconstructed baryon likelihoods and
are not observational morphology inputs.  They are also not an
ECT-derived lensing operator.  Sigma_cl(Sigma_b) is pointwise
monotone, so argmax Sigma_cl=argmax Sigma_b is a tautological sanity
check, not a morphology reproduction or selection.  E1+E2 are
conditional, E4 is a hand-set proxy, and E3 is retained only as an
explicitly Open candidate sensitivity proxy; no orientation stress
tensor is derived.
...
def amplitude_corrections(cl,A0):
    ...
    E4=0.20
    return A0*(1+E1E2)*(1+E3)*(1+E4),E1E2,E3,E4,phi_bar

results={}
cnames=list(CLUSTERS.keys())
print("="*60)
print("Independent Level-C benchmark — monotone baryonic-peak preservation sanity check")
print("="*60)
for name,cl in CLUSTERS.items():
    Sbcg,Sgas,Sb,nu2d,Seff=build_maps(cl)
    xp,yp=find_peak(Seff)
    xp_b,yp_b=find_peak(Sb)
    argmax_preserved=(xp==xp_b and yp==yp_b)
    ...
    A_final,E1E2,E3,E4,phi_bar=amplitude_corrections(cl,A0)
    results[name]=dict(Sbcg=Sbcg,Sgas=Sgas,Sb=Sb,nu2d=nu2d,Seff=Seff,
        xp=xp,yp=yp,xp_b=xp_b,yp_b=yp_b,argmax_preserved=argmax_preserved,
        d_star=d_star,d_gas=d_gas,peak_on_bcg=peak_on_bcg,
        A0=A0,A_final=A_final,E1E2=E1E2,E3=E3,E4=E4,phi_bar=phi_bar,cl=cl)
    pred='BCG' if peak_on_bcg else 'gas'
    print(f"{name:<14} argmax_preserved={argmax_preserved} "
          f"context_peak={pred} d*={d_star:.0f}kpc "
          f"A0proxy={A0:.3f} Afinproxy={A_final:.3f}")
...
fig1=plt.figure(figsize=(7.8,8.8))
gs1=gridspec.GridSpec(3,2,figure=fig1,hspace=0.48,wspace=0.58)
...
imshow_grey(ax_sb,rb['Sb'][xl:xr,yl:yr],r'(a) $\Sigma_b$ [toy Gaussian input]',
            r'$M_\odot\,\mathrm{kpc}^{-2}$')
ax_sb.text(0.04,0.06,'toy gas input dominates',transform=ax_sb.transAxes,fontsize=7,style='italic')
...
ax_nu.set_title(r'(b) Independent Level-C $\nu_{\rm cl}$'); ax_nu.set_xlabel('$x$ [kpc]'); ax_nu.set_ylabel('$y$ [kpc]')
...
imshow_grey(ax_ef,rb['Seff'][xl:xr,yl:yr],r'(c) $\Sigma_{\rm cl}=\nu_{\rm cl}\Sigma_b$ [monotone; peak preserved]',
            r'$M_\odot\,\mathrm{kpc}^{-2}$')
ax_ef.plot(rb['xp'],rb['yp'],marker='x',color='white',ms=8,mew=1.5,zorder=9)
ax_ef.text(rb['xp']+42,rb['yp']+55,r'$\arg\max\Sigma_{\rm cl}$',fontsize=7,color='white',bbox=dict(fc='0.1',ec='none',alpha=0.45,pad=1.5))
...
ax_pr.plot(xs,rb['Seff'][xl:xr,jc],color='k',ls='-',lw=1.5,label=r'$\Sigma_{\rm cl}$')
...
# No imported MOND-level guide in this independent cluster benchmark.
for bar,val in zip(bars,vals):
    ax_am.text(bar.get_x()+bar.get_width()/2,val+0.013,f'{val:.3f}',
               ha='center',va='bottom',fontsize=7.5)
# No empirical deficit shading: obs_amp_ratio is a hand-set benchmark input.
ax_am.text(3.6,1.03,'unit guide',fontsize=7,ha='right',va='bottom')
# Removed stale MOND-level label.
ax_am.set_xticks(np.arange(4)); ax_am.set_xticklabels(labels,fontsize=6.5)
ax_am.set_ylabel(r'illustrative amplitude proxy $A_{\rm proxy}$')
ax_am.set_title('(e) Proxy-normalisation sensitivity (Bullet)')
...
ax_sl.axvspan(0.03,0.08,color='0.82',alpha=0.55,label='illustrative input range')
ax_sl.set_xlabel(r'illustrative proxy amplitude $\bar\phi_{\rm proxy}$')
ax_sl.set_ylabel(r'$E_3$ [\%]')
ax_sl.set_title(r'(f) $E_3$ sensitivity via $X_{\rm cand}$ (Open; no derived $\Theta_{\mu\nu}$)')
ax_sl.legend(fontsize=7,framealpha=0.85); ax_sl.grid(True,alpha=0.25,lw=0.5)
ax_sl.set_ylim(-0.5,10.5)
# Open-proxy status is carried by the panel title; no derived range claim.
fig1.text(0.5,0.005,
    r'Markers = centres of hand-set toy BCG/gas Gaussians; masses, widths, and offsets are not reconstructed baryon likelihoods'+'\n'+r'Bullet inputs: $g^\dagger_{\rm cl}=1.2\times10^{-10}\,\mathrm{m\,s^{-2}}$, hand-set $A_{\rm obs}^{\rm proxy}=3.0$; amplitudes are illustrative proxies'+'\n'+r'$\Sigma_{\rm cl}(\Sigma_b)$ is monotone: peak preservation is a tautological sanity check, not morphology evidence',
...
gs2=gridspec.GridSpec(3,2,figure=fig2,left=0.08,right=0.98,top=0.96,bottom=0.10,
...
for i,(name,ax) in enumerate(zip(cnames,axes_pr)):
    ...
    ax.plot(xs2,r2['Seff'][xl2:xr2,jc],color='k',ls='-',lw=1.5,label=r'$\Sigma_{\rm cl}$')
...
ax_bud.bar(x4+w/2,Afins,w,color='0.35',edgecolor='k',lw=0.7,label=r'$A_{\rm fin}^{\rm proxy}$ (mixed-status sensitivity)')
ax_bud.axhline(1.0,color='k',ls='-',lw=1.2)  # unit guide only
for xi,af in zip(x4,Afins):
    ax_bud.text(xi+w/2,af+0.015,f'{af:.3f}',ha='center',va='bottom',fontsize=7.5)
ax_bud.set_xticks(x4); ax_bud.set_xticklabels(['Bullet','MACS\nJ0025','El\nGordo','A520'],fontsize=8.5)
ax_bud.set_ylabel(r'illustrative $A_{\rm proxy}$'); ax_bud.set_title('(i) Proxy-normalisation sensitivity',fontsize=9)
...
ax_bud.text(3.85,1.03,'unit guide',fontsize=7.5,ha='right',va='bottom')

ax_tab=fig2.add_subplot(gs2[2,1])
ax_tab.axis('off'); ax_tab.set_title('(j) Sanity-check outputs',fontsize=9,pad=4)
col_labels=['Cluster',r'$\arg\max$',r'$d_*^{\rm cl}$',r'$A_0^{\rm proxy}$',r'$A_{\rm fin}^{\rm proxy}$']
cell_text=[]
for name in cnames:
    r2=results[name]; cl2=r2['cl']
    pred='BCG' if r2['peak_on_bcg'] else 'gas'
    d_cl_display=f"{r2['d_star']:.0f} kpc" if pred=='BCG' else 'N/A'
    cell_text.append([name,pred,d_cl_display,
                      f"{r2['A0']:.3f}",f"{r2['A_final']:.3f}"])
col_widths=[0.30,0.14,0.18,0.19,0.19]
...
fig2.text(
    0.5, 0.018,
    r'Exploratory system-specific $g^\dagger_{\rm cl}$ inputs '
    r'[Bullet, MACS J0025, El Gordo, A520] '
    r'$=[1.2,1.0,1.4,1.0]\times10^{-10}\,\mathrm{m\,s^{-2}}$; '
    r'not a common-parameter prediction.' + '\n' +
    r'Hand-set $A_{\rm obs}^{\rm proxy}=[3.0,2.5,3.5,2.0]$ and '
    r'hand-set Gaussian masses/widths/offsets; not reconstructed '
    r'baryon likelihoods or observational morphology evidence.' + '\n' +
    r'$A_0^{\rm proxy},A_{\rm fin}^{\rm proxy}$ are illustrative '
    r'normalisation sensitivities, not empirical mass residuals; '
    r'exact peak preservation is a monotonicity tautology.',
    ha='center', fontsize=6.1, style='italic')
p2=os.path.join(OUTDIR,'fig_cluster_suite_budget.png')
fig2.savefig(p2); print(f'Saved: {p2}'); plt.close(fig2)
meta = {
    'schema_version': 1,
    'status': 'PROPOSAL_ONLY_EXPLORATORY_LEVEL_C',
    'interpolation': 'nu_cl(y)=sqrt((1+sqrt(1+4/y^2))/2)',
    'relation_to_other_interpolations': (
        'inverse-boost form of the same alternative mu_dim; '
        'independent of practical SPARC/G and no transfer claimed'
    ),
    'system_specific_g_dagger_cl_m_s2': {
        name: float(CLUSTERS[name]['g_dag']) for name in cnames
    },
    'system_specific_A_obs_proxy_input': {
        name: float(CLUSTERS[name]['obs_amp_ratio']) for name in cnames
    },
    'generator_alias_for_A_obs_proxy': 'obs_amp_ratio',
    'common_parameter_claim': False,
    'amplitude_proxy_status': (
        'illustrative normalisation sensitivity from hand-set '
        'obs_amp_ratio; not M_cl/M_obs or an empirical residual'
    ),
    'toy_gaussian_input_status': (
        'hand-set masses, widths, and offsets; not '
        'source-reconstructed baryon likelihoods'
    ),
    'observational_morphology_claim': False,
    'toy_map_inputs': {
        name: {
            'z_context': float(CLUSTERS[name]['z']),
            'bcg_gaussians': [
                [float(value) for value in row]
                for row in CLUSTERS[name]['bcg']
            ],
            'gas_gaussians': [
                [float(value) for value in row]
                for row in CLUSTERS[name]['gas']
            ],
        } for name in cnames
    },
    'orientation_E3': 'Open X_cand sensitivity proxy; no derived Theta_mu_nu',
    'peak_preservation_status': (
        'Sigma_cl(Sigma_b) is strictly monotone; argmax equality is '
        'a tautological sanity check, not morphology evidence'
    ),
    'all_argmax_preserved': all(
        bool(results[name]['argmax_preserved']) for name in cnames
    ),
    'proxy_outputs': {
        name: {
            'argmax_preserved': bool(results[name]['argmax_preserved']),
            'argmax_Sigma_b_kpc': [
                float(results[name]['xp_b']), float(results[name]['yp_b'])
            ],
            'argmax_Sigma_cl_kpc': [
                float(results[name]['xp']), float(results[name]['yp'])
            ],
            'd_star_raw_kpc': float(results[name]['d_star']),
            'd_star_display': (
                f"{results[name]['d_star']:.0f} kpc"
                if results[name]['peak_on_bcg'] else 'N/A (gas peak)'
            ),
            'A0_proxy_display_3dp': round(float(results[name]['A0']), 3),
            'A_fin_proxy_display_3dp': round(float(results[name]['A_final']), 3),
            'phi_proxy': float(results[name]['phi_bar']),
            'E12_conditional': float(results[name]['E1E2']),
            'E3_Xcand_open': float(results[name]['E3']),
            'E4_hand_set_proxy': float(results[name]['E4']),
            'A0_proxy_raw': float(results[name]['A0']),
            'A_fin_proxy_raw': float(results[name]['A_final']),
        } for name in cnames
    },
    'audit_fixed_inputs': {
        'epsilon': 0.01, 'E2': 0.01, 'a_for_E3': 1.0, 'E4': 0.20
    },
    'bullet_A_fin_proxy_display_3dp': round(
        float(results['Bullet']['A_final']), 3
    ),
}
pm=os.path.join(OUTDIR,'cluster_benchmark_inputs.json')
