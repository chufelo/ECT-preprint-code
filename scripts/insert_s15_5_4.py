#!/usr/bin/env python3
"""Phase 1 integration of §15.5.4."""
import hashlib
from pathlib import Path

LATEX = Path("/Users/chufelo/Documents/Physics/VDT/ECT/LaTex")
PREPRINT = LATEX / "ECT_preprint.tex"
SNIPPET = LATEX / "proposals/drafts_for_integration/04_s15_5_4_LATEX_SNIPPET.tex"
...
# Anchor A: replace §15.5.3 END marker with §15.5.4 content + new END marker
anchor_A_old = (
    "% ==================================================================\n"
...
anchor_A_new = (
    "% ==================================================================\n"
    "% END §15.5.3 (rebuilt). §15.5.4 follows below.\n"
    "% ==================================================================\n"
    "\n"
    + snippet.rstrip() + "\n\n"
)
...
# Post-invariants
assert text3.count("\\label{subsec:eps_excluded_rebuilt}") == 1

# Legacy-number guard on new §15.5.4 block
new_block_start = text3.find("subsec:eps_excluded_rebuilt")
new_block_end = text3.find("END §15.5.4 (rebuilt)")
assert new_block_start != -1 and new_block_end != -1
new_block = text3[new_block_start:new_block_end]
for forbidden in [
    "0.010", "0.012", "$0.01$", "3\\%", "2.85", "2.90", "2.8\\%",
    "69~\\text", "69\\,\\mathrm", "69 km", "69~km",
    "epsilon_diagram.html", "fig_epsilon_constraints",
    "few-per-cent",
    # GPT round 9: forbidden phrases explicitly removed
    "most naturally read",
    "independent of detailed proxy kernel",
    "Placeholder removed",
    # Old text had these specific intervals that GPT round 9 removed
    "-0.065", "-0.018", "-0.031", "-0.009",
    "\\kappa_A \\in [2,",
]:
    assert forbidden not in new_block, f"FORBIDDEN token in new §15.5.4: {forbidden!r}"
print("All legacy-number + GPT-r9-forbidden-phrase guards OK.")
...
print("OK: §15.5.4 integrated; annotation updated.")
