...
digraph ECT {
  ...
  BH [label=<Black Holes &amp; Strong Field<BR/><FONT POINT-SIZE="9">Info destruction excluded [A]<BR/>Shell · BH thermo · Area law [A/B]<BR/>Page curve [B/Open]</FONT>>];
  ...
  BORN [label=<Born Rule<BR/><FONT POINT-SIZE="9">RP + Gleason route   [B]</FONT>>];
  ...
  GAUGE [label=<Gauge &amp; Matter Sector<BR/><FONT POINT-SIZE="9">U(1) → photon · Charge lattice [A]   ·   SU(2) → W<SUP>±</SUP>, Z   ·   Higgs (v₂ ≈ 246 GeV)<BR/>Fermions: O(3) spinor reps   ·   SU(3): open   ·   3 gen.: open<BR/>Neutrino: seesaw route (C)   ·   M<SUB>R</SUB><SUP>geom</SUP> ~ 10<SUP>10</SUP> GeV   [A/B/C/Open]</FONT>>,
        fillcolor="#ddd"];
  S0 -> GAUGE [xlabel=<phase>];
  TOPO -> GAUGE [xlabel=<spinor>];
  FIFTH -> GAUGE [style=invis]; /* pull gauge down */

  /* ===== DISCRETE-SYMMETRY ARCHITECTURE (F3, §9.4) ===== */
  CPT [label=<Discrete-Symmetry Architecture<BR/><FONT POINT-SIZE="9">P, T, C · Conditional CPT via OS reconstruction<BR/>Branch-pair on S³ · 𝓛₅ → CPT-odd phenom. op.<BR/>[A/B; Open for full nonlinear closure]</FONT>>,
         fillcolor="#d3d3d3"];
  GAUGE -> CPT;
