...
\begin{description}[leftmargin=2.8cm, style=nextline]
\item[Standard QM.] Outcome selection is described through the collapse postulate: upon measurement the state projects onto an eigenstate of the observable, with probability $|\langle\text{eigenstate}|\psi\rangle|^2$.
\item[ECT (physical picture).] The detector is treated as part of the full physical system and therefore belongs to the global boundary-value problem.  In the coherent regime ($\Gamma_{\rm loop}\ll 1$) multiple winding sectors coexist without a definite outcome; environmental coupling to many modes drives the decoherence functional $\Gamma[q]$ to grow; when $\Gamma_{\rm loop}\gtrsim 1$ the off-diagonal interference terms are suppressed and an effectively classical single-branch outcome emerges.  Effective ``collapse'' is read as the subsystem manifestation of this Euclidean-to-Lorentzian regime transition.
\item[What this explains.] Why a definite outcome appears at all without any external ``collapse'' postulate; why the statistical weights inherit a Born-type form (the Hilbert/projector structure produced by the OS reconstruction makes the weights quadratic in the amplitudes, with conditional Gleason uniqueness once standard closure assumptions are admitted).
\item[Status.] Structural measurement reading, not yet a complete detector-level theorem; a full microscopic derivation of outcome selection from the bare condensate action remains open (\emph{Level~Open}, OP-Q8).
\end{description}
...
Gravitational decoherence realises a DP-type leading scale within the condensate reduced-state architecture; ECT-specific content lies in the condensate-origin interpretation, absence of a freely fitted collapse scale, and possible subleading corrections (nanosphere prediction: $\tau\sim 38$\,ms).
