...
The same closure that reproduces the SPARC rotation curves and the radial acceleration relation runs into a sharp and well-known tension on a small, unusual class of observed objects: the so-called \emph{dark-matter-deficient ultra-diffuse galaxies}. The reference cases are NGC~1052--DF4 (often written DF4) and FCC~224 --- faint, extended dwarfs of roughly $10^8$ solar masses and a few kiloparsecs in size --- whose observed internal stellar motions are much colder than the galactic closure of~\eqref{eq:pop_interp} would normally predict at that mass and size. In rough terms, the closure would read a typical line-of-sight velocity dispersion of order $20$~km\,s$^{-1}$ for such objects at the baseline $g^\dagger$, while DF4 shows about $6$--$8$~km\,s$^{-1}$ and FCC~224 shows a similarly low value. Crucially, a matched-mass companion system, NGC~5846-UDG1, with nearly identical stellar mass, size, and group-scale environment, does sit on the standard radial acceleration relation at around $17$~km\,s$^{-1}$. Two complementary diagnostic readouts make the tension concrete. The first, Fig.~\ref{fig:pop_udg_regime}, places the reference objects in the regime plane of the ECT galactic closure --- the same qualitative picture used elsewhere to describe when dynamics are Newton-like, RAR-normal, or deep-MOND. It shows where the closure \emph{expects} each system to sit (using only its observed baryons) versus where the \emph{observed kinematics actually place it}. For DF4 and FCC~224 the closure expects them on the MOND-like branch but the data put them near Newton; the matched control NGC~5846-UDG1 sits on the RAR-normal strip with essentially no mismatch.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{fig_udg_regime_diagram.pdf}
\caption{Positioning of the five UDG reference objects in the ECT regime plane of the galactic closure. Open diamonds mark where the closure would place each object given its observed baryons alone (via the interpolation law on which the SPARC rotation-curve fits are based). Filled circles mark the acceleration inferred from the observed velocity dispersion. For NGC~1052--DF4 and FCC~224 the two markers disagree by a large margin: the closure predicts enhanced MOND-like acceleration, but the kinematic readout is close to baryon-only Newton. The matched-mass DM-rich control NGC~5846-UDG1 shows no such mismatch. (\emph{Level~B/Open.})}
\label{fig:pop_udg_regime}
\end{figure}

Physically, the observed DF4/FCC~224 kinematics look Newton-like: the stars behave as if there were essentially no enhancement beyond the visible baryons inside the probed radius. In the language of the present closure, the deep-MOND branch of~\eqref{eq:pop_interp} is \emph{not} activated at those objects, even though their baryonic surface densities would normally put them in exactly that regime. The minimal environment rule used for $g^\dagger$ depends only on the ambient density, and that alone cannot naturally distinguish DF4 from NGC~5846-UDG1, because their group-scale environments are comparable. This is a sharp empirical discriminant for the present Level-B galactic closure, and it is an honest open question --- not a solved consequence of ECT.

The quantitative diagnostic across the five reference objects is compactly summarised in Fig.~\ref{fig:pop_udg}: for each object the plot shows the value $\Xi_{\rm req}$ that the closure would need in order to reproduce the observed dispersion.

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{fig_udg_stress_test.pdf}
