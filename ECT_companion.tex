...
In standard QM, one solves $H\psi = E\psi$ and obtains
$E_n = -13.6/n^2$\,eV together with the probability densities
$|\psi_{nlm}|^2$.
ECT gives a structurally richer picture of the same system.

The atom exists not in a fundamentally given Lorentzian spacetime
but in the symmetry-broken condensate background that supports the
emergent spacetime and quantum dynamics (Layer~1: background).
The electron is described as a spinorial field compatible with the
condensate geometry and the emergent Lorentzian sector; it is not
yet derived as a collective excitation of the bare bosonic
condensate (Layer~2: electron).
The proton is treated as a localised composite source in the
emergent gauge sector; its full microscopic internal origin
(colour structure) is not yet derived from first principles
(Layer~3: proton).
The electromagnetic binding interaction is interpreted within the
emergent $U(1)$ gauge sector, rather than being introduced in ECT
as an independent fundamental ingredient external to the condensate
framework (Layer~4: electromagnetic bond).
The atom as a whole is a self-consistent bound configuration
involving the protonic source, the electronic spinorial field,
the $U(1)$ gauge connection, and the surrounding
condensate-supported spacetime and quantum
background (Layer~5: bound state).

The ground state~$1s$ is the lowest regular, node-free,
spherically symmetric bound mode consistent with these structural
ingredients.
At the structural level, discrete levels may be understood as
arising because only certain regular, phase-consistent,
single-valued solutions are admissible---in
harmony with the winding/single-valuedness conditions of the
coherent branch:
$\oint dw \cdot p_w = 2\pi S_0\,n$ (\emph{Level~B}).

\emph{From Dirac to Schr\"odinger.}
The full structural description of hydrogen is first spinorial and
gauge-coupled.
The Schr\"odinger orbital, with its familiar $|\psi_{nlm}|^2$
probability densities, is the non-relativistic envelope limit---the
regime where velocities, binding energies, and spin-dependent
relativistic corrections are small compared with the rest scale.
Within ECT, the familiar Schr\"odinger description is therefore
treated as a low-energy effective level of description rather than
the deepest structural level.

\emph{What remains open.}
The current framework does not yet derive hydrogenic fine structure,
the Lamb shift, hyperfine splitting, or the numerical value of
$\alpha_{\rm fs}=1/137$.
These are open derivational targets (\emph{Level~Open}).

\textbf{The double-slit experiment.}
In standard QM, $\psi = \psi_1 + \psi_2$ and
$P = |\psi_1 + \psi_2|^2$.
In ECT, the interference pattern is interpreted as arising from a
connected solution of the underlying elliptic boundary-value
problem, whose effective
Lorentzian projection contains contributions associated with both
slit channels.
The interference is a property of the four-dimensional field
configuration, not merely of the effective probability-amplitude
description
(\emph{Level~A/B}).
The slit arrangement constrains the families of admissible
phase-coherent condensate configurations; the detected pattern
emerges from their coherent recombination.
On this reading, the wavefunction is the effective envelope of a
deeper phase-coherent condensate response.

\emph{Loss of interference and decoherence.}
When which-path information is coupled to the environment, the
relevant phase coherence is lost: the influence functional from the
environmental modes suppresses the off-diagonal (cross-slit)
terms.
In the ECT language, this is precisely the transition
$\Gamma_{\rm loop} \ll 1 \to \Gamma_{\rm loop} \gtrsim 1$
described in the decoherence section---interference vanishes not
because of a mysterious collapse, but because the
condensate-plus-environment response ceases to remain
phase-coherent (\emph{Level~A/B}).

\textbf{The Aharonov--Bohm effect.}
In standard QM, a charged particle acquires an observable phase
shift $\Delta\varphi = (e/\hbar)\oint A_\mu\,dx^\mu$ even in
regions where the electromagnetic field strength vanishes.
In the observational regime this is the standard $\hbar$-based form;
within the ECT coherent branch it corresponds to the same
phase-holonomy structure after the identification $S_0=\hbar$.
In ECT, this is especially natural: the compact phase $\theta(X)$
is already a primary structural variable of the coherent branch,
and the gauge connection $A_A$ arises as its local compensating
field.
The AB phase shift is therefore a direct manifestation of a
nontrivial holonomy of the emergent $U(1)$ connection around a
closed loop.
Phase and loops are fundamental ingredients of the ECT quantum
sector, so the Aharonov--Bohm effect is not an odd addition to the
theory but exactly the kind of phenomenon one should expect when
quantum dynamics emerges from phase-sensitive loop
structure (\emph{Level~B: interpretive}).
