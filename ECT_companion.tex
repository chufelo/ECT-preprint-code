...
\caption{The ECT Hubble-tension alleviation mechanism. The evolving effective gravitational coupling $G_{\rm eff}(z) > G_N$ at high redshift shifts the late-time $H_0$ scale upward by $\sim 2$--$3$\,km/s/Mpc within the benchmark closure, alleviating but not resolving the full $5$--$6$\,km/s/Mpc Planck-SH0ES tension. (\emph{Sign: Level~A structural prediction; magnitude: Level~B benchmark-closure indicator.})}
...
ECT offers a structural alleviation mechanism (Fig.~\ref{fig:pop_hubble}). At high redshift, the condensate was slightly less screened, meaning $G_{\rm eff}(z) > G_N$. Specifically, $G_{\rm eff}(z) \approx G_N(1+z)^{2\varepsilon}$ with $\varepsilon \approx 0.01$, where $\varepsilon$ is a late-time \emph{benchmark} slope imported from the present ECT closure rather than a first-principles derived constant. Within this benchmark, the present-epoch closed-form benchmark relation gives an upward shift of the late-time $H_0$ scale by about $3\%$, or roughly $2$--$3$\,km/s/Mpc in absolute units. This alleviates the Planck-SH0ES tension of order $5$--$6$\,km/s/Mpc but does \emph{not} resolve it: at best it closes about one-third of the observed discrepancy. The present paper also does not yet implement the full CMB-inferred early+late $H_0$ pipeline that would be required for a genuine tension-resolution claim.
...
\item Hubble tension \emph{alleviation} (not resolution): $\Delta H_0 \sim +2$--$3$\,km/s/Mpc, covering roughly one-third of the $\sim 5$--$6$\,km/s/Mpc gap (\emph{sign: A; magnitude: B, benchmark closure}).
...
determinations of $H_0$ that can test the benchmark
...
Using the same benchmark parameter $\varepsilon=0.01$ as in the
Hubble-tension discussion, a semi-analytic ISW--tracer overlap proxy
