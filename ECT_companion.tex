...
\caption{Part~III derivation logic, updated with the QG/decoherence upgrade layer. The coherent branch produces: Hilbert-space structure, wave equations, Born rule, decoherence, PES, vacuum effects, BH thermodynamics, and measurement theory, together with the reduced-state upgrade layer comprising the hybrid-consistency architecture and two-level ontology (\S\ref{sec:pop_hybrid_consistency}), the gravity-induced-entanglement operational criterion (\S\ref{sec:pop_gie}), the $\phi$ vs $\delta n_A$ mediator taxonomy (\S\ref{sec:pop_mediator}), the BMS/memory/soft-graviton infrared sector (\S\ref{sec:pop_ir_soft}), the islands/QES structural mapping with the critical shell (discussed within the BH section), and the open orientation-sector entangling-mediator task (OP-Q22). Colours indicate derivation levels.}
...
\item \textbf{The measurement problem.} Decoherence and PES remove the need for a fundamental collapse postulate, but the full Born-rule closure remains conditional (\emph{B}; unconditional closure open).
