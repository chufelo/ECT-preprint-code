...
\item \textbf{Fine-structure constant.} After colour completion the fine-structure problem is recast as a concrete electroweak kinetic/mixing target: external one-loop benchmark running implies moderate ultraviolet gauge-sector normalisations, structurally tied to the same condensate architecture that also underlies $G_N$ and $\hbar$, while a full first-principles derivation remains open (\emph{B/C}).
\end{enumerate}
