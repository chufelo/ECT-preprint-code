...
A helpful intuition is that an entangled pair is not two independent objects connected by a mysterious faster-than-light wire. It is one non-factorised condensate pattern seen through two reduced laboratory descriptions. Alice's and Bob's systems are distant in the emergent Lorentzian branch, but their reduced states belong to the same coherent Euclidean configuration. A measurement at Alice's side does not send a signal to Bob; it selects a local reduced description compatible with the shared configuration. Bob's local statistics remain unchanged, which is why no signalling is possible. But the joint correlations can still violate Bell inequalities, because the relevant structure is not a set of independent local hidden variables assigned separately to Alice and Bob --- it is a single non-factorised four-dimensional configuration whose reduced descriptions appear in two distant laboratories. This is the sense in which ECT is ``local'' for signals but not ``local'' in the Bell-hidden-variable sense.
...
The Wick rotation $t\to-i\tau$ is a formal procedure that is always
available, but its physical content depends on the question being
asked. For amplitudes between two classically separated minima it
generates the bounce / instanton saddle that controls tunnelling. For
a system with a single classical minimum it does not produce a bounce
saddle; it instead yields the Euclidean structure underlying the
ordinary stationary states (e.g.\ ground-state localisation around the
minimum). The Euclidean machinery is therefore a universal
reconstruction tool, not a recipe for converting every Lorentzian
problem into a barrier-crossing problem.
...
\textbf{Common pre-geometric architecture of interactions:} Gravity, electromagnetism, the weak sector, the colour sector, and the predicted director coupling are all routed through the same condensate architecture, but with different logical status. The Abelian gauge sector is derived from bundle covariance (Level~A); a preferred chiral $SU(2)$ sector is structurally selected by the oriented condensate (Level~B+); the colour sector requires extended internal structure. This is broader than conventional Grand Unification, which never includes gravity. The weak-sector parity asymmetry acquires a structural route from the condensate orientation, but the exact maximally parity-violating Standard-Model pattern remains an open derivation problem.
