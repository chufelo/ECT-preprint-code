...
$n^A$ & Ordered direction in 4D Euclidean space (unit vector, $n^A n_A=1$); the kinematic constraint underlying the Level~A resolution of the cosmological-constant catastrophe. \\
...
\item \textbf{Numerical value of $\rho_\Lambda$ (reformulated OP-$\Lambda$3).} The 120-orders cosmological-constant \emph{catastrophe} is resolved at Level~A by the emergent unimodular decoupling theorem: because the kinetic-tensor determinant $\sqrt{-\det K^{AB}}=\beta^2/c_*$ is fixed, the classical vacuum offset $V(\phi_0)$ does not source emergent gravity (unconditional), and the zero-derivative loop contribution decouples under condition $H_\Lambda$. What remains open is the narrower Level~C question of the observed \emph{value} $\rho_\Lambda$: its natural scaling $\sim M_{\rm Pl}^2 H_0^2$ matches observation at order-of-magnitude, but the dimensionless coefficient is not yet computed from first principles. A possible pseudo-Goldstone realisation may exist if an ultralight pseudo-Goldstone sector with $m_G\sim H_0$ and decay-constant scale of order $\phi_0$ is realised in the cosmological regime.
...
\item \textbf{Why $3+1$ dimensions and why no fundamental anyons?} The $O(4)\to O(3)$ transition singles out exactly three spatial directions and one temporal seed. In $d=3$ the exchange topology permits only the ordinary boson/fermion split; fundamental anyons are excluded in the primary branch. Standard physics postulates $d=3+1$ without explanation (\emph{A}).
...
\item \textbf{Quantum gravity.} ECT offers a structural route in which gravity appears as a condensate response and the graviton as an excitation of the ordered medium on a Euclidean background; the full quantum-gravity closure remains open (\emph{A/B}).
...
Lorentzian signature from $O(4) \to O(3)$ SSB; universal causal cone ($c_* = c$); the equation hierarchy (Euclidean $\to$ KG $\to$ Schr\"odinger); arrow of time from retarded decoherence; BTFR slope exactly~4; structural non-inflationary routes to the horizon, flatness, and primary-monopole problems; exclusion of fundamental information destruction at the condensate-state level, with constructive black-hole evaporation still open.
