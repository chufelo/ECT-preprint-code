...
\phantomsection\label{thm:cc_decoupling}%
...
\noindent
\textit{Note on identification conventions.}
Equation~\eqref{eq:sqrt_g_const} uses the densitized-inverse-metric
identification of~\eqref{eq:K_geff_identification}, under which
$\sqrt{-\det K^{AB}}=\beta^2/c_*$ coincides with
$\sqrt{-g_{\mathrm{eff}}}$ up to a field-independent prefactor.
Other sections of the paper treat $K^{AB}$ as the ordinary inverse
metric $G^{AB}_{\mathrm{eff}}$ itself (\S\ref{sec:effective_metric}),
under which $\sqrt{-g_{\mathrm{eff}}}=c_*/\beta^2$; the two conventions
agree at the canonical benchmark $(\alpha,\beta)=(2,1)$.
The physically invariant content of part~(i) is that $\det K^{AB}$
is $n$-independent; clauses~(ii)--(v) of this theorem depend only on
this fact together with the unit constraint $n^A n_A=1$, and therefore
go through identically under either identification convention.
...
\item[(v)] \textbf{Object B decoupling (conditional on $H_\Lambda$).}
\emph{If} the full quadratic fluctuation operator contains
...
labeled $H_\Lambda$, stated precisely in Sec.~\ref{subsec:Hlambda_cc} -- the
...
\end{equation}
and does not source local emergent gravitational dynamics.
Conversely, generic violations of $H_\Lambda$ --- for example a
scalar prefactor $Z(\phi)$ --- reintroduce the coupling,
transferring the UV sensitivity into the scalar/radial-sector
effective potential rather than into a direct
cosmological-constant term; see Sec.~\ref{subsec:Hlambda_cc} for
details.
\end{description}
...
Part (v) combines (ii) and (iv) under the prefactor assumption $H_\Lambda$.
...
conditional on $H_\Lambda$. Neither feeds directly into Object~C in this sector. Beyond-leading
corrections to the ansatz~\eqref{eq:KAB_cc} are tracked
separately as Open Problem~O5 (Sec.~\ref{subsec:cc_open}).

\subsection{The condition $H_\Lambda$ and its logical status}
\label{subsec:Hlambda_cc}

Theorem~\ref{thm:cc_decoupling}(v) decouples Object~B from Object~C conditionally on an
...
\paragraph{Condition $H_\Lambda$.} Let $\chi(x)$ denote a generic low-energy
...
\end{equation}
\textbf{$H_\Lambda$} states that the kinetic matrix decomposes as
\begin{equation}
...
\emph{$H_\Lambda$ holds.} Theorem~\ref{thm:cc_decoupling}(v) applies and Object~B decouples exactly
...
\emph{$H_\Lambda$ fails with a scalar prefactor $Z(\phi)$.} Then
...
\paragraph{Object~A is unaffected by $H_\Lambda$.} Regardless of whether $H_\Lambda$
holds, Object~A (the classical $V(\phi_0)$) decouples, because its
decoupling follows from the Jacobian identity alone (Theorem~\ref{thm:cc_decoupling}(iv)).
\paragraph{Status.} $H_\Lambda$ is a condition on the full quadratic
...
treats Theorem~\ref{thm:cc_decoupling}(v) as a result conditional on $H_\Lambda$.
...
UV baseline $V(\phi_0)$ (which is decoupled by Theorem~\ref{thm:cc_decoupling}(iv)) but its
...
grounding for the dark-energy relation of
Sec.~\ref{sec:mp_late_cosmology}: with
$\rho_{\mathrm{kin}} = \tfrac12 Z_q \dot q^2$ and
$\rho_{\mathrm{cond}} = V_{\mathrm{IR}}(q)$, the exact slow-branch
relation is $w_0=(r_0-1)/(r_0+1)$,
$r_0\equiv\rho_{\mathrm{kin}}/\rho_{\mathrm{cond}}$, linearising to
\begin{equation}
w_0 \;\simeq\; -1 + \frac{2\rho_{\mathrm{kin}}}{\rho_{\mathrm{cond}}}\,.
\end{equation}
The benchmark value $w_0\approx-0.83$ corresponds to
$f_0\approx0.087$ in the linearised thawing benchmark (while the
exact scalar relation gives
$r_0=(1+w_0)/(1-w_0)\approx0.095$), and is therefore an explicit
linearised instance of the general
...
{\small
\begin{longtable}{p{3.2cm} p{3.5cm} p{3.2cm} p{4.0cm}}
...
admissible configuration space [Theorem~\ref{thm:cc_decoupling}(i)--(iii)].
\item Object~A (classical $V(\phi_0)$) decouples unconditionally from
local emergent gravitational dynamics [Theorem~\ref{thm:cc_decoupling}(iv)].
\item Under condition $H_\Lambda$, Object~B (zero-derivative loop
contribution) likewise decouples [Theorem~\ref{thm:cc_decoupling}(v)]; if $H_\Lambda$ fails, the UV
...
sector, conditionally on $H_\Lambda$ for Object~B and unconditionally for
Object~A.
...
\item Condition $H_\Lambda$ is expected to hold to leading order in the ECT
...
We claim: (a)~the $10^{120}$ UV-vs-IR catastrophe of the standard
framework is structurally resolved at Level~A in the leading
ordered-branch sector of ECT, in the sense that the classical
vacuum baseline $V(\phi_0)$ does not gravitate
(Theorem~\ref{thm:cc_decoupling}(iv); rigorous within the ordered-branch ansatz), and,
conditional on the single assumption $H_\Lambda$, the zero-derivative one-loop
vacuum contribution likewise does not gravitate (Theorem~\ref{thm:cc_decoupling}(v));
(b)~the classical-level statement is unconditional and does not
require any cancellation between unrelated UV and IR quantities;
(c)~Weinberg's no-go theorem in its standard form does not apply
to ECT; (d)~the surviving infrared source parametrises as
$\rho_\Lambda\sim c_\Lambda M_{\rm Pl}^2 H_0^2$, matching observation
at the order-of-magnitude level; (e)~a plausible microphysical
realisation of the infrared scaling is the pseudo-Goldstone sector
of the $O(4)\to O(3)$ breaking.

We do not claim: (a)~a first-principles derivation of the numerical
coefficient $c_\Lambda$, or equivalently of the observed numerical
value $\rho_{\Lambda,\mathrm{obs}}$ -- this is a separate Level~C
determination problem listed as~O2;
(b)~that condition $H_\Lambda$ has been verified against the full ECT
Lagrangian beyond the leading ordered-branch ansatz -- this is~O1;
(c)~that the Ng--van Dam non-renormalization theorem has been
formally transferred to the emergent setting -- this is~O3;
(d)~that the pseudo-Goldstone realisation of
Sec.~\ref{subsec:goldstone_cc} is established as the cosmological
source; it is a plausible candidate only.

\paragraph{Structural separation of two distinct questions.}
We emphasise that the classical ``cosmological constant problem'' of
the standard framework -- the catastrophic $10^{120}$ mismatch
between a UV baseline $\sim M_{\rm Pl}^4$ and the observed
$\rho_\Lambda^{\rm obs}\sim 10^{-47}\,\mathrm{GeV}^4$ -- is
structurally separated in ECT from the \emph{value} of
$\rho_{\Lambda,\mathrm{obs}}$ itself. The former is a UV-channel
statement about what does \emph{not} gravitate and is at Level~A in
the leading sector (unconditional for the classical baseline;
conditional on $H_\Lambda$ for the zero-derivative loop). The latter is an
infrared determination problem for a specific number and is at
Level~C. Conflating the two -- as is often done in the standard
literature -- underestimates the structural content of the
decoupling theorem and overstates the logical scope of the
remaining $O(1)$ coefficient question.
...
\begin{description}
\item[O1.] \textit{Verification of $H_\Lambda$ in the full quadratic operator.}
...
zero-derivative sector. If $H_\Lambda$ holds, Object~B decouples exactly; if
$H_\Lambda$ fails, classify the residual UV sensitivity as a scalar
...
\item[O5.] \textit{Beyond leading ansatz.} Extend Theorem~\ref{thm:cc_decoupling} to
...
The classical cosmological constant catastrophe -- the
$10^{120}$-level mismatch between a UV baseline $\sim M_{\rm Pl}^4$
and the observed $\rho_\Lambda^{\rm obs}\sim 10^{-47}\,\mathrm{GeV}^4$
in the standard framework -- is \emph{resolved at Level~A} in the
leading ordered-branch sector of ECT. The emergent metric
determinant is fixed by the kinematic constraint $n^A n_A = 1$ to
$\sqrt{-g_{\rm eff}} = \beta^2/c_*$, and the classical baseline
$V(\phi_0)$ does not source the local emergent gravitational
dynamics (Theorem~\ref{thm:cc_decoupling}(iv)). Conditional on the single spectral
assumption $H_\Lambda$, the same fixed-determinant structure absorbs the
zero-derivative one-loop vacuum contribution (Theorem~\ref{thm:cc_decoupling}(v)). The
direct $10^{120}$-level cancellation between a UV vacuum baseline
and the observed cosmological term is therefore absent in this
sector: no cross-scale cancellation between unrelated quantities is
invoked at all.

The observed numerical value $\rho_{\Lambda,\mathrm{obs}}$ is a
distinct, infrared determination problem. Its natural parametric
scaling $\rho_\Lambda \sim c_\Lambda M_{\mathrm{Pl}}^2 H_0^2$ is
consistent with observation at the order-of-magnitude level and
admits a plausible realization through the pseudo-Goldstone sector,
though neither the scaling coefficient nor the realization is
established as a derived prediction (Level~C). The outstanding
technical tasks for this infrared sector -- $H_\Lambda$ verification to lift
the $H_\Lambda$-conditional status of Theorem~\ref{thm:cc_decoupling}(v), explicit FRW coefficient
computation, and a non-renormalization analogue of Ng--van Dam --
are listed as Open Problems O1--O5 and together constitute the
reformulated OP-$\Lambda$3 programme.
