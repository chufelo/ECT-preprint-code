...
\large\textcolor{red}{[SUPERSEDED --- see status notice below]}}
\author{Internal working document --- NOT for publication (historical)}
\date{2026-04-20}
...
This is an \emph{internal historical proposal document} from the
integration-planning phase of the UDG topic.  The recommendations
inside (``no standalone subsection'', ``~150-word paragraph only''
etc.) do \textbf{not} reflect the final integration that was
actually adopted.  The final integration in the main preprint is a
full standalone subsection~16.5 (``Dark-matter-deficient
ultra-diffuse galaxies as a stress test of the current closure'')
in the main text, together with technical Appendix~AR.  For the
authoritative published formulation see the main ECT preprint.
}}
\end{center}
