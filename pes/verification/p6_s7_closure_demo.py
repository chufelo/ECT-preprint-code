#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# P6 S7 capstone (Claude): T2 full P26 over-closure loop — reconstruct J_W from the width corner,
# then PREDICT the dephasing corner and the KK-shift corner from the reconstruction;
# T3 Cauchy-Schwarz cross-spectrum positivity detector (practical G4 test).
import csv, math
import numpy as np
...
print('T2 P26 over-closure loop (ground truth: C = 0.37, omega_c = 3.4, T = 0.72; coordinate monitor F = 1):')
...
for t in (0.5, 5.0, 50.0):
    ...
    print('  dephasing corner PREDICTED: t = %5.1f: Phi_rec/Phi_true = %.6f' % (t, r))
RR.append({'section':'T2','case':'dephasing corner','value':'worst %.2e'%worstD,'check':'<1e-2','note':'predicted from reconstruction'})
assert worstD < 1e-2
...
print('  KK-shift corner PREDICTED: Omega_0 = 0.9: shift_rec/shift_true = %.6f' % rs)
RR.append({'section':'T2','case':'shift corner','value':'%.6f'%rs,'check':'1','note':'bosonic mirror kernel (R19)'})
assert abs(rs - 1) < 1e-2
print('  => the triangle OVER-CLOSES: one corner reconstructs the kernel; the other two are predictions (P26 live).')
